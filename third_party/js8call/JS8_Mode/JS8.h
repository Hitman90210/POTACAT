...
// POTACAT local modification (NOTES.md): upstream expressed this as a
// constexpr lambda returning a capturing lambda, which MSVC rejects with
// C2326 (upstream builds with MinGW and never met MSVC). Same values, same
// call syntax, still constexpr-evaluable (call sites use it in constexpr
// initializers).
inline constexpr auto COSTAS_TABLE =
...
constexpr Array const &array(Type type) {
    return COSTAS_TABLE[static_cast<std::underlying_type_t<Type>>(type)];
}
} // namespace Costas
