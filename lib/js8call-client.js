...
class Js8CallClient extends EventEmitter {
  ...
  /**
   * Tell JS8Call what frequency it is on.
   *
   * With `Rig=None` — the correct setting on a Flex Direct station, because
   * POTACAT owns the CAT link — JS8Call has no idea where the radio is. It
   * keeps showing the old dial after POTACAT retunes its slice, which means its
   * spots, its logging and its band activity are all filed under the wrong
   * band (K3SBP 2026-08-07: "the radio QSYd but the JS8Call app did not").
   *
   * Verb confirmed present in the JS8Call-improved 2.4.0 binary alongside
   * RIG.GET_FREQ / RIG.FREQ; params.DIAL is the dial frequency in Hz.
   */
  setDialFreq(hz) {
    const f = Math.round(Number(hz) || 0);
    if (!f) return false;
    return this.send({ type: 'RIG.SET_FREQ', params: { DIAL: f, OFFSET: 0 } });
  }
