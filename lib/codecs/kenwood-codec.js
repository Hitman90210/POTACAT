...
class KenwoodCodec extends EventEmitter {
  ...
  _parseMessage(msg) {
    // IF; status (Kenwood 38-char layout): P10 (index 31) = RX VFO digit
    // (0=VFO A, 1=VFO B, 2=memory), P12 (index 33) = split flag. Checked
    // BEFORE the FA branch on principle (no prefix overlap today, but IF is
    // the more specific message). Yaesu's IF; layout differs — gated off.
    if (msg.startsWith('IF') && !this._isYaesu() && msg.length >= 34 && /^\d{11}$/.test(msg.slice(2, 13))) {
      const vfoDigit = msg.charAt(31);
      ...
      this.emit('split', msg.charAt(33) === '1');
      return;
    }
