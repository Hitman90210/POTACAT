...
class KenwoodCodec extends EventEmitter {
  ...
  _parseMessage(msg) {
    ...
    if (msg.startsWith('IF') && !this._isYaesu() && msg.length >= 37 && /^\d{11}$/.test(msg.slice(2, 13))) {
      ...
      this.emit('split', msg.charAt(32) === '1');
      return;
    }
    ...
    // VFO B frequency — polled instead of FA when the IF; readback says the
    // radio is on VFO B, so the display follows the VFO the operator hears.
