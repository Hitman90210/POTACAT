...
const RIGCTLD_LEVEL_KINDS = { STRENGTH: 'smeter', SWR: 'swr', ALC: 'alc' };
...
function rigctldPendingKind(line) {
  ...
  if (verb === 'u') return parts[1] === 'NB' ? 'nb' : 'other';
...
class RigctldCodec extends EventEmitter {
  ...
  /** Record a command so a later RPRT can be blamed on the right one. */
  _notePending(line) {
    const kind = rigctldPendingKind(line);
    if (!kind) return;
    this._pending.push(kind);
  ...
  _resolvePending(kind) {
    const i = this._pending.indexOf(kind);
  ...
  _parseLine(line) {
    ...
    if (/^RPRT\s+-?\d+/.test(line)) {
      ...
      const blame = this._pending.shift();
      const failed = code !== 0;
      if (blame === 'nb') {
        this._expectNb = false;
