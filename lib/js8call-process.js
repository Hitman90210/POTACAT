...
/**
 * Where the SmartSDR DAX control panel might live, as {dir, depth} pairs for
 * the caller to list. Two layouts, and knowing only one of them is how POTACAT
 * launched a dead install (K3SBP 2026-08-06):
 *
 *   v3.x / v4.0-4.1  %ProgramFiles%\FlexRadio Systems\SmartSDR v3.10.10\DAX\DAX.exe
 *   v4.2+            %ProgramFiles%\SmartSDR v4.2.20\DAX.exe
 *
 * Old versions leave their directories behind when v4.2+ installs elsewhere,
 * so finding *a* DAX.exe is not the same as finding the working one.
 */
function daxSearchSpecs({ platform = process.platform, env = process.env } = {}) {
  if (platform !== 'win32') return [];
  const out = [];
  for (const r of [env['ProgramFiles'], env['ProgramFiles(x86)']].filter(Boolean)) {
    out.push({ base: path.join(r, 'FlexRadio Systems'), sub: 'DAX' });
    out.push({ base: r, sub: '' });
  }
  return out;
}

/** The version a SmartSDR path advertises, as comparable numbers. */
function smartSdrVersion(p) {
  const m = String(p || '').match(/SmartSDR[\s_v]*(\d+(?:\.\d+)*)/i);
  return m ? m[1].split('.').map(Number) : [];
}

/**
 * The newest DAX.exe from a list of found paths.
 *
 * By VERSION, not by path order or string order: "SmartSDR v4.1.5" sorts above
 * "SmartSDR v4.2.20" as text, and the two layouts do not even share a parent
 * directory, so nothing about where a file sits says how new it is. An orphan
 * left by an upgrade looks exactly like a live install until you read its
 * version — and the orphan is missing DLLs, so launching it opens a crash
 * dialog that blames FlexRadio for POTACAT picking the wrong one.
 */
function pickNewestDax(paths) {
  const list = (paths || []).filter(Boolean);
  if (!list.length) return '';
  return list.slice().sort((a, b) => {
    const va = smartSdrVersion(a), vb = smartSdrVersion(b);
