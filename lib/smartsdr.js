...
class SmartSdrClient extends EventEmitter {
  ...
  async createSlice({ freq, mode = '', ant = '' } = {}) {
    ...
    // A success status with an unreadable payload means a slice may well exist
    // that we cannot address or clean up. Refuse rather than guess at 0.
    if (!Number.isInteger(idx)) throw new Error(`slice create returned no index (payload "${payload}")`);
    console.log(`[SmartSDR] created slice ${idx} (${freq} MHz ${mode || ''})`.trim());
    return idx;
  }
