...
class SmartSdrClient extends EventEmitter {
  constructor() {
    ...
    this._sock = null;
    this._seq = 1;
    // seq -> {resolve, reject, timer} for _sendAwait(). Cleared on disconnect
    // so a pending caller learns the socket died instead of timing out.
    this._awaiting = new Map();
    // idx -> {dax, freq}, maintained from `sub slice all` status lines.
    this._sliceInfo = new Map();
    // Slices POTACAT made for something else (JS8Call). They must never become
    // "the slice we follow": creating one makes it active, and in bound mode
    // POTACAT would jump its tuning and its DAX route onto the new receiver —
    // which is exactly what happened the first time (K3SBP 2026-08-07).
    this._excludedSlices = new Set();
    // Slice indexes known BEFORE an in-flight `slice create`. While this is
    // set, any slice index outside it is ours-for-another-app and is excluded
    // the instant it is announced. Excluding after createSlice() resolves is
    // too late: it resolves ON that announcement, and the follow logic runs
    // first (K3SBP 2026-08-07 — POTACAT followed slice 2 anyway).
    this._createBaseline = null;
    this._buf = '';
    ...
    this._sliceMuted = {};           // idx -> monitor mute state (audio_mute); muted also kills the DAX tap
  ...
  _doConnect() {
    ...
    sock.on('connect', () => {
      ...
      this._txDaxOn = null;       // transmit status dax= (null until first status)
    ...
    sock.on('close', () => {
      ...
      this._sock = null;
      this._cwBound = false;
      // Fail anything waiting on a reply. Leaving them to time out would make a
      // caller wait 5 s to learn something already known, and a slice-create
      // caller that cannot tell "failed" from "pending" creates a second slice.
      for (const [, w] of this._awaiting) { clearTimeout(w.timer); w.reject(new Error('radio connection closed')); }
  ...
  _handleLine(line) {
    ...
    if (rMatch) {
      ...
      // Generic awaited command (see _sendAwait). Checked first: these are
      // explicit one-off requests, and nothing else claims their sequence.
      const waiter = this._awaiting.get(seq);
      if (waiter) {
        this._awaiting.delete(seq);
        clearTimeout(waiter.timer);
        waiter.resolve({ status, payload: (line.split('|')[2] || '').trim() });
        return;
      }
  ...
  _setupMeterUdp() {
    ...
    this._fwdMeterId = null;   // forward power, for "did we radiate"
  ...
  _parseMeterPacket(buf) {
    ...
    for (let i = 0; i < numMeters; i++) {
      ...
      } else if (meterId === this._fwdMeterId) {
        // Forward power, same /128 scaling as the other TX-bridge meters.
        this.emit('fwd-power', rawValue / 128.0);
      }
    }
  }
    ...
    if (line.includes('.src=') && line.includes('.nam=')) {
      ...
      for (const [id, m] of Object.entries(meters)) {
        ...
        if (m.src && m.nam) {
          ...
          // Forward power. "Did we actually radiate" is otherwise a question
          // only answerable by watching a meter on the radio while a 13-second
          // frame goes out — and PSKReporter silence cannot distinguish a dead
          // band from a dead transmitter.
          if ((m.nam === 'FWDPWR' || m.nam === 'FWD_PWR' || m.nam === 'PWR') && !this._fwdMeterId) {
            this._fwdMeterId = numId;
            console.log(`[SmartSDR] Forward-power meter: id ${numId} (nam=${m.nam} src=${m.src})`);
          }
        } else if (m.nam) {
  ...
  _parseTransmitStatus(line) {
    ...
    // dax= is the radio's TX-audio-source switch (SmartSDR's "DAX" button on
    // the P/CW transmit panel): 1 = transmit audio comes from the DAX TX
    // stream, 0 = from the mic input. POTACAT's direct VITA-49 dax_tx feed
    // only modulates when this is 1 — with it 0 the radio keys a dead
    // carrier (the June 2026 bound-mode symptom; self mode "worked" only on
    // radios whose TX profile had DAX persisted on). Tracked here so main
    // can assert it for the duration of a POTACAT transmission and restore.
    if (kv.dax != null) this._txDaxOn = kv.dax === '1';
  ...
  /** Last-known transmit dax= state: true/false, or null before the first
   *  transmit status of the session (sub tx all delivers one at connect).
   *  Normalized so pre-connect (init lives in the per-connect reset) and
   *  post-connect-no-status both read as null. */
  get txDaxOn() { return this._txDaxOn == null ? null : this._txDaxOn; }
  ...
  _parseSliceStatus(line) {
    ...
    // Keep a live picture of which slices exist and which DAX channel each one
    // feeds. Needed to hand a NEW slice a channel nobody is using: on a Flex a
    // DAX channel with no slice bound to it is a device that opens and carries
    // silence, which is indistinguishable from a dead band until you notice it
    // never decodes anything (2026-08-06).
    if (/(?:^| )removed(?:$| )/.test(body) || get('in_use') === '0') {
      this._sliceInfo.delete(idx);
    } else {
      const prev = this._sliceInfo.get(idx) || {};
      const dax = get('dax');
      this._sliceInfo.set(idx, {
        dax: dax === null ? prev.dax : Number(dax),
        freq: get('RF_frequency') || prev.freq,
      });
    }
    ...
    // A slice POTACAT created for another application is never the operator's
    // slice, however the radio flags it. Creating one marks it active, and
    // without this POTACAT follows it — moving its tuning, its frequency
    // readout and its DAX route onto JS8Call's receiver.
    // A slice appearing during our own `slice create` is ours, for another
    // application. Claim it here, in the same synchronous pass that would
    // otherwise adopt it.
    if (this._createBaseline && !this._createBaseline.has(idx)) {
      this._excludedSlices.add(idx);
    }
    if (this._excludedSlices.has(idx)) return;
    ...
    // Monitor mute state. On 4.2.x `audio_mute=1` also kills the DAX tap (the
    // DAX RX is taken from the slice monitor mix), so a slice muted by the
    // operator or another GUI client means POTACAT's RX audio + JTCAT/SSTV
    // decoders go SILENT with no error to catch. Emit changes so main can say
    // so out loud instead of us overriding the operator's mute (KD0RC #67).
    const amute = get('audio_mute');
    if (amute != null) {
      const muted = amute === '1';
      if (this._sliceMuted[idx] !== muted) {
        this._sliceMuted[idx] = muted;
        this.emit('slice-mute', { index: idx, muted });
      }
    }
  ...
  /** Monitor-mute state of a slice as last reported by the radio, or
   *  undefined if the radio hasn't told us yet. */
  sliceMuted(idx) { return this._sliceMuted[idx]; }
  ...
  /**
   * Send a command and resolve with its reply payload.
   *
   * Most commands here are fire-and-forget, and the two that were not
   * (`client gui`, `client bind`) each grew their own `_xxxSeq` field. A third
   * is where that stops scaling — `slice create` is useless without its reply,
   * because the radio chooses the slice number.
   *
   * Resolves {status, payload}; rejects on timeout or a dropped socket, never
   * hangs. A caller that cannot tell "failed" from "still waiting" ends up
   * creating a second slice on top of the first.
   */
  _sendAwait(cmd, timeoutMs = 5000) {
    return new Promise((resolve, reject) => {
      const seq = this._send(cmd);
      if (seq === null) return reject(new Error('not connected'));
      const timer = setTimeout(() => {
        this._awaiting.delete(seq);
        reject(new Error(`no reply to "${cmd}" after ${timeoutMs}ms`));
      }, timeoutMs);
      this._awaiting.set(seq, { resolve, reject, timer });
    });
  }
  ...
  /**
   * Create a slice receiver and resolve with the index the RADIO assigned.
   *
   * Syntax per FlexRadio's own API wiki (TCPIP-slice):
   *   slice create [freq=<MHz>][pan=<streamID>][ant=<port>][mode=<mode>][clone_slice=<n>]
   * and the reply payload is the new slice number. The radio chooses it — a
   * caller that assumes "the next one" will eventually address someone else's
   * slice, which on a 4-slice 8600 is a matter of when, not if.
   *
   * @param {{freq:number, mode?:string, ant?:string}} o freq in MHz
   * @returns {Promise<number>} the new slice index
   */
  async createSlice({ freq, mode = '', ant = '' } = {}) {
    const parts = [];
    if (freq) parts.push(`freq=${Number(freq).toFixed(6)}`);
    if (ant) parts.push(`ant=${ant}`);
    if (mode) parts.push(`mode=${mode}`);

    // Which slices existed BEFORE, so the new one can be identified by
    // difference. The API wiki says the reply carries the assigned slice
    // number; this radio (8600, SmartSDR v4.2.20) answers `R<seq>|0|` with an
    // EMPTY payload and announces the slice on the status stream instead.
    // Refusing on that basis created a real slice and then disowned it —
    // orphaned on a radio with four (K3SBP 2026-08-07). The status stream is
    // authoritative either way, so use the payload only as a shortcut.
    const before = new Set(this._sliceInfo.keys());
    this._createBaseline = before;
    // Re-subscribe so the radio replays slice state. In bound mode POTACAT can
    // reach this point before its first `sub slice all` replay has arrived,
    // and the radio only sends slice status on CHANGE — so a create whose
    // announcement was missed leaves nothing else to wait for.
    this._send('sub slice all');
    try {
      const { status, payload } = await this._sendAwait(`slice create ${parts.join(' ')}`.trim());
      if (status !== 0) throw new Error(`radio refused slice create (status 0x${status.toString(16)})`);

      const quick = parseInt(payload, 10);
      if (Number.isInteger(quick)) {
        this._excludedSlices.add(quick);
        console.log(`[SmartSDR] created slice ${quick} (${freq} MHz ${mode || ''})`.trim());
        return quick;
      }
      const idx = await this._awaitNewSlice(before);
      this._excludedSlices.add(idx);
      console.log(`[SmartSDR] created slice ${idx} (${freq} MHz ${mode || ''}) — index from status, reply was empty`.trim());
      return idx;
    } finally {
      this._createBaseline = null;
    }
  ...
  /**
   * The first slice index to appear that was not in `before`.
   *
   * Polls the status-fed map rather than racing a one-shot listener: the
   * announcement can arrive before the command reply does, in which case a
   * listener registered afterwards would wait forever for an event already past.
   */
  _awaitNewSlice(before, timeoutMs = 8000) {
    const started = this._nowMs();
    return new Promise((resolve, reject) => {
      const tick = () => {
        const fresh = [...this._sliceInfo.keys()].filter((i) => !before.has(i));
        if (fresh.length) return resolve(Math.min(...fresh));
        if (this._nowMs() - started > timeoutMs) {
          // Report what we DID know. "Never announced it" alone cannot
          // distinguish "the radio made nothing" from "status never reached
          // us" — and those need opposite fixes.
          const known = [...this._sliceInfo.keys()].sort((x, y) => x - y);
          return reject(new Error(
            `slice created but the radio never announced it (slices seen before: [${[...before].sort().join(',')}], now: [${known.join(',')}])`));
        }
        setTimeout(tick, 100);
      };
      tick();
  ...
  _nowMs() { return Number(process.hrtime.bigint() / 1000000n); }

  /** Remove a slice. Documented form is `slice r <n>`, not `slice remove`. */
  removeSlice(idx) {
    if (!Number.isInteger(idx)) return;
    console.log(`[SmartSDR] removing slice ${idx}`);
    this._send(`slice r ${idx}`);
  }

  /** Slice indexes the radio has told us about (from `sub slice all`). */
  get sliceIndexes() {
    return [...this._sliceInfo.keys()].sort((a, b) => a - b);
  }

  /** Mark a slice as not-ours-to-follow (see _excludedSlices). */
  excludeSlice(idx) { if (Number.isInteger(idx)) this._excludedSlices.add(idx); }
  unexcludeSlice(idx) { this._excludedSlices.delete(idx); }

  /** A slice's dial frequency in Hz, or 0 if unknown. */
  sliceFreqHz(idx) {
    const info = this._sliceInfo.get(Number(idx));
    const mhz = info && parseFloat(info.freq);
    return mhz ? Math.round(mhz * 1e6) : 0;
  }

  /** The DAX channel a given slice feeds, or 0 if none/unknown. */
  sliceDaxChannel(idx) {
    const info = this._sliceInfo.get(Number(idx));
    return info && Number.isInteger(info.dax) ? info.dax : 0;
  }

  /** DAX channels currently bound to a slice — i.e. the ones carrying audio. */
  get usedDaxChannels() {
    const out = new Set();
    for (const info of this._sliceInfo.values()) {
      if (Number.isInteger(info.dax) && info.dax > 0) out.add(info.dax);
    }
  ...
  /**
   * Select the radio's TX audio source: true = the DAX TX stream (POTACAT's
   * direct VITA-49 dax_tx feed), false = the mic input. This is the global
   * `transmit set dax=` switch the June 2026 dead-carrier investigation
   * predicted — NOT the per-slice dax_tx field, which is never to be touched
   * (setting it killed the slice's DAX RX; fully reverted 2026-06-25).
   */
  setTxDax(on) {
    this._send(`transmit set dax=${on ? 1 : 0}`);
  }
  ...
  // FM squelch threshold (0-100) for the given slice. Flex slices carry
  // squelch_level; squelch=1 arms it (audio muted below threshold).
  setSquelch(idx, pct) {
    const v = Math.max(0, Math.min(100, Math.round(pct)));
    this._send(`slice set ${idx} squelch=1 squelch_level=${v}`);
  }
