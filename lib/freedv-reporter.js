...
class FreedvReporterClient extends EventEmitter {
  ...
  disconnect() {
    ...
    if (this._ws) {
      // Stale-socket guard (crash 2026-08-15): close() on a CONNECTING ws
      // does NOT cancel its in-flight TCP attempt — the eventual ETIMEDOUT
      // (~20s later) still fires 'error' on this ws, reaching a client whose
      // consumer may have removeAllListeners()'d it by then. An 'error'
      // event with no listener kills the PROCESS, so detach our handlers
      // and swallow anything the dying socket says after this point.
  ...
  /**
   * Emit 'error' ONLY when someone is listening. A reporting sidecar must
   * be constitutionally unable to crash the app: an EventEmitter 'error'
   * with zero listeners throws to uncaughtException, which is exactly how
   * an unreachable qso.freedv.org took down the whole app (2026-08-15).
   */
  _emitError(err) {
    if (this.listenerCount('error') > 0) this.emit('error', err);
    else console.warn('[FreeDV Reporter] (unheard) error:', err && err.message);
  }
