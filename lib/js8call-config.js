...
/** Do two audio-device strings name the same device? Windows, JS8Call and
 *  Chromium all spell the same card slightly differently ("Microphone (USB
 *  Audio CODEC)" vs "USB Audio CODEC"), so compare on the distinctive part
...
function readJs8Settings(ini) {
  ...
  return {
    ...
    catSerial: String(v('CATSerialPort') || v('CATUSBPort') || '').trim(),
 ...
 * Collisions are real on EVERY rig — two apps cannot share one radio — but only
 * a FlexRadio has slices and DAX channels. Naming them to an IC-7300 operator
 * is nonsense they cannot act on, so `rigFamily` selects the vocabulary and the
 * Flex-specific checks are simply not run elsewhere. (CLAUDE.md's rig-scoped-UI
 * rule; K3SBP 2026-08-06.)
 *
 * @param {string} [o.rigFamily]         'flex' | 'icom' | … from lib/rig-family.js
 * @param {number} [o.potacatSlicePort]  Flex CAT shim port POTACAT drives (5002-5005)
 * @param {number} [o.potacatDaxChannel] Flex DAX RX channel POTACAT captures on
 * @param {string} [o.potacatCatPath]    non-Flex: the serial port POTACAT holds
 * @param {string} [o.potacatAudioIn]    non-Flex: the audio input POTACAT captures
 ...
 */
function diagnoseJs8Config({
  ini, rigFamily = '', potacatSlicePort = 0, potacatDaxChannel = 0,
  potacatCatPath = '', potacatAudioIn = '', needTx = false,
} = {}) {
  ...
  const isFlex = String(rigFamily).toLowerCase() === 'flex';

  if (isFlex) {
    // Slices and DAX exist only here. A Flex can genuinely run both apps at
    // once — on separate slices — so the fix is a real one.
    ...
    // One receiver, one serial port. There is no second slice to move to, so
    // the honest advice is that they take turns — not a fix, a fact.
    if (potacatCatPath && s.catSerial &&
        s.catSerial.toUpperCase() === String(potacatCatPath).toUpperCase()) {
      found.push({
        severity: 'conflict', code: 'cat-port-collision',
        message: `JS8Call and POTACAT are both set to control the radio on ${s.catSerial}. Only one program can hold a serial port.`,
        fix: 'Let POTACAT keep the radio and set JS8Call\'s radio to "None", or close POTACAT\'s connection while you use JS8Call.',
      });
    }
    if (potacatAudioIn && s.soundIn &&
        sameDevice(s.soundIn, potacatAudioIn)) {
      found.push({
        severity: 'conflict', code: 'audio-in-collision',
        message: `JS8Call captures ${s.soundIn}, the same input POTACAT is using.`,
        fix: 'Only one program can capture a sound card. Give JS8Call its own input, or stop POTACAT decoding while you use JS8Call.',
      });
    }
  }
