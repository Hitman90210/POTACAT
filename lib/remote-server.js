...
`;
  ...
  _handleConnection(ws, req) {
    ...
    if (!requiresAuth) {
      ...
      // The legacy token path never grew the newer feeds inline — which is
      // the drift the shared helpers exist to end. Hydrate them here too.
      this._sendJs8Hydration(ws);
      this._sendActivityHydration(ws);
  ...
  _handleMessage(ws, msg, req) {
    ...
    if (msg.type === 'auth') {
      ...
      if (authenticated) {
        ...
        // JS8 inbox + the idle-activity feeds (shared helpers — see their
        // header for why these are never inlined per-path).
        this._sendJs8Hydration(ws);
        this._sendActivityHydration(ws);
        if (this._directoryData.nets.length || this._directoryData.swl.length) {
          this._sendTo(ws, { type: 'directory', nets: this._directoryData.nets, swl: this._directoryData.swl });
  ...
  _sendActivityHydration(ws) {
    if (this._activityState) this._sendTo(ws, { type: 'activity-state', ...this._activityState });
  ...
  broadcastActivityState(state) {
    ...
    if (this.hasClient()) this._sendTo(this._client, { type: 'activity-state', ...state });
  }
