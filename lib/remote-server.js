  ...
  setAltHosts({ tsHost, cloudHost } = {}) {
    ...
    if (this._client && this._client.readyState === WebSocket.OPEN && this._client._authenticated) {
      ...
      this._sendTo(this._client, {
        ...
        tsCertPublic: !!this._tlsCertPublic,
        fingerprint: this._certFingerprint(),
      });
    }
  }
...
`;
  ...
  _handleConnection(ws, req) {
    ...
    this._sendTo(ws, protocol.buildServerHello({
      ...
      capabilities: ['diagnostic-snapshot', 'spot-target', 'rx-gain-sync', 'tx-drive', 'js8'],
    ...
    if (!requiresAuth) {
      ...
      this._client = ws;
      this._sendTo(ws, { type: 'auth-ok', colorblindMode: !!this._colorblindMode, settings: this._remoteSettings, cwAvailable: this._cwEnabled, cwPaddleAvailable: this._cwPaddleAvailable, cwPaddleReason: this._cwPaddleUnavailableReason, vfoLocked: !!this._vfoLocked, tsHost: this._altHosts.tsHost, cloudHost: this._altHosts.cloudHost, tsCertPublic: !!this._tlsCertPublic, fingerprint: this._certFingerprint(), stationCallsign: (this._remoteSettings && this._remoteSettings.myCallsign ? String(this._remoteSettings.myCallsign).toUpperCase() : null) });
  ...
  _handleMessage(ws, msg, req) {
    ...
    if (msg.type === 'auth') {
      ...
      if (authenticated) {
        ...
        // JS8: the inbox as it stands. State first (running/heartbeat gate
        // the phone's controls), then conversations, then the heard rail.
        if (this._js8State) this._sendTo(ws, { type: 'js8-state', ...this._js8State });
        if (this._js8Threads) this._sendTo(ws, { type: 'js8-threads', ...this._js8Threads });
    ...
    switch (msg.type) {
      ...
      case 'js8-thread-open':
        // Read-only browsing — allowed for guests.
        this.emit('js8-thread-open', { id: msg.id });
        break;
      ...
      case 'cw-text':
        ...
        if (msg.text && typeof msg.text === 'string') {
          this.emit('cw-text', { text: msg.text, wpm: msg.wpm });
        }
        break;
  ...
  async _authenticatePass(ws, msg, req) {
    ...
    try {
      ...
      // ungated), and browsing needs an inbox to browse. Without this a
      // guest arrived at a blank JS8 surface until the next live push,
      // which on a quiet band is many minutes (mobile team, 2026-08-09 —
      // docs/desktop-asks/js8-mobile-gaps.md). Same order as the paired
      // path: state first, so controls render disabled-with-a-reason
      // rather than unknown.
      if (this._js8State) this._sendTo(ws, { type: 'js8-state', ...this._js8State });
      if (this._js8Threads) this._sendTo(ws, { type: 'js8-threads', ...this._js8Threads });
      if (this._js8Heard && this._js8Heard.length) this._sendTo(ws, { type: 'js8-heard', list: this._js8Heard });
  ...
  broadcastJs8Threads(payload) {
    ...
    // not a stale "this one just changed" hint from an hour ago.
    this._js8Threads = { list: payload.list, unread: payload.unread };
    if (this.hasClient()) this._sendTo(this._client, { type: 'js8-threads', ...payload });
  }
  ...
  /** One thread's full content — the reply to js8-thread-open. */
  sendJs8Thread(thread) {
    if (this.hasClient()) this._sendTo(this._client, { type: 'js8-thread', thread });
  }
