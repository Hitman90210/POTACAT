  ...
  _handleMessage(ws, msg, req) {
    ...
    if (msg.type === 'auth') {
      ...
      if (authenticated) {
        ...
        this._sendTo(ws, { type: 'status', ...this._radioStatus });
        // Router + inbox hydrate right after status on EVERY path — the
        // client routes on activity-state before content lands, which is
        // only true if nothing piles up in front of it.
        this._sendActivityHydration(ws); // the router, before any content
        this._sendJs8Hydration(ws);
        if (this._activatorState) {
          this._sendTo(ws, { type: 'activator-state', ...this._activatorState });
    ...
    switch (msg.type) {
      ...
      case 'js8-log-prefill':
        ...
        // prefill itself would read a DM's contents anyway.
        if (ws._passSession) { this._js8Refuse(ws, msg.reqId); break; }
        this.emit('js8-log-prefill', { id: msg.id });
        break;
      case 'set-freedv':
