...
/**
 * Is this text already addressed to someone?
 *
 * Deliberately narrower than the INBOUND parser's first-token match: that one
 * reads traffic already known to be directed, while this decides whether to ADD
 * a destination, and "HELLO THERE" must not look addressed to HELLO.
 */
function isAddressed(text) {
  const t = String(text || '').trim();
  // A leading @GROUP, or a leading token followed by a colon. The colon must
  // come before any space, so "MEET AT 10:30" is a sentence, not a message
  // addressed to MEET.
  return /^@[A-Z0-9/]{2,}(\s|$)/i.test(t) || /^[A-Z0-9/]{2,}:/i.test(t);
}

/**
 * Address a message the way JS8 does — which is not one way but two.
 *
 * Real traffic (see threadIdFor): `KC1QKM: K3SBP HELLO` for a callsign,
 * `@HB HB FN20` for a group. Using a colon for both sends group messages to a
 * station named "@HB", and pasting a heartbeat into an addressed box produces
 * `@HB: @HB HB FN20`. Already-addressed text is returned untouched.
 */
function composeDirected(to, body) {
  const b = String(body || '').trim();
  if (!b) return '';
  if (isAddressed(b)) return b;
  const t = String(to || '').trim().toUpperCase();
  if (!t) return b;
  return isGroupTarget(t) ? `${t} ${b}` : `${t}: ${b}`;
}

module.exports = {
  Js8Threads, isHeartbeatText, isGroupTarget, threadIdFor,
  isAddressed, composeDirected,
  GROUPS, MAX_THREADS, MAX_MESSAGES,
};
