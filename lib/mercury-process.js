 ...
 * @returns {string[]}
 */
function buildMercuryArgs(settings, iniPath, audio) {
  const c = mercuryConfig(settings);
  // `audio` (from resolveMercuryAudio in lib/mercury-audio-bridge.js) overrides
  // the raw settings sound system + devices — that's how the FIFO bridge selects
  // `-x fifo` with the FIFO paths. Omitted → plain device config from settings.
  const soundSystem = audio ? audio.soundSystem : c.soundSystem;
  const inputDevice = audio ? audio.inputDevice : c.inputDevice;
  const outputDevice = audio ? audio.outputDevice : c.outputDevice;
  const args = ['-C', iniPath, '-p', String(c.basePort), '-b', String(c.broadcastPort)];
  if (soundSystem && soundSystem !== 'auto') args.push('-x', soundSystem);
  if (inputDevice) args.push('-i', inputDevice);
  if (outputDevice) args.push('-o', outputDevice);
 ...
 * @returns {string}
 */
function buildMercuryIni(settings = {}, audio) {
  const c = mercuryConfig(settings);
  const soundSystem = audio ? audio.soundSystem : c.soundSystem;
  const inputDevice = audio ? audio.inputDevice : c.inputDevice;
  const outputDevice = audio ? audio.outputDevice : c.outputDevice;
...
/**
 * Parse the stdout of `mercury -z` (device discovery). Mercury prints:
 *   playback devices:
 *   device: name: '<name>'  id: '<id>'  default: <flag>
 *   capture devices:
 *   device: name: '<name>'  id: '<id>'  default: <flag>
 * We return each list with the human name (for display) and id (what `-i`/`-o`
 * actually take). The default flag is whatever ffaudio prints ("1"/"0"/…).
 * @param {string} stdout
 * @returns {{capture: Array<{name,id,isDefault}>, playback: Array<{name,id,isDefault}>}}
 */
function parseSoundcardList(stdout) {
  const out = { capture: [], playback: [] };
  let section = null;
  for (const raw of String(stdout || '').split(/\r?\n/)) {
    const line = raw.trim();
    if (/^playback devices:/i.test(line)) { section = 'playback'; continue; }
    if (/^capture devices:/i.test(line)) { section = 'capture'; continue; }
    const m = line.match(/^device:\s*name:\s*'(.*)'\s*id:\s*'(.*)'\s*default:\s*(\S+)\s*$/i);
    if (m && section) {
      out[section].push({ name: m[1], id: m[2], isDefault: /^(1|yes|true|default)$/i.test(m[3]) });
    }
  }
  return out;
}
...
/**
 * Which sound system to pass to `mercury -x <ss> -z` for discovery. Device
 * listing needs a concrete backend, so resolve 'auto' (and the no-device
 * backends) to the platform default.
 */
function mercuryDiscoverySoundSystem(settings = {}, platform = process.platform) {
  const ss = settings.mercurySoundSystem;
  if (ss && !['auto', 'fifo', 'null', 'shm'].includes(ss)) return ss;
...
/**
 * Why didn't Mercury's TNC come up? Nearly always because a configured audio
 * device no longer exists: Mercury can't open it, audio init fails, the ARQ
 * listener never starts, and it sits there alive but useless. DAX endpoints
 * are the usual culprit — SmartSDR deletes them when the DAX app closes — but
 * any device picked once and later unplugged does it.
 *
 * K3SBP's own station, 2026-08-05: Mercury running, only the broadcast port
 * open, and BOTH configured device ids absent from Mercury's list of 37. The
 * launch had looked completely normal in the log for weeks.
 *
 * @param {object} o
 * @param {string} [o.inputDevice]   configured capture id (from the resolved audio)
 * @param {string} [o.outputDevice]  configured playback id
 * @param {object} [o.devices]       parseSoundcardList() output
 * @returns {{missing: Array<{role:string, id:string}>, checked: boolean}}
 *   checked=false when there was nothing to check against (empty enumeration),
 *   so callers don't report "device missing" from a failed enumeration.
 */
function findMissingMercuryDevices({ inputDevice = '', outputDevice = '', devices = null } = {}) {
  const capture = (devices && Array.isArray(devices.capture)) ? devices.capture : [];
  const playback = (devices && Array.isArray(devices.playback)) ? devices.playback : [];
  if (!capture.length && !playback.length) return { missing: [], checked: false };
  const has = (list, id) => list.some((d) => d && d.id === id);
  const missing = [];
  // An empty setting means "let Mercury choose", which is never missing.
  if (inputDevice && capture.length && !has(capture, inputDevice)) {
    missing.push({ role: 'input', id: inputDevice });
  }
  if (outputDevice && playback.length && !has(playback, outputDevice)) {
    missing.push({ role: 'output', id: outputDevice });
  }
