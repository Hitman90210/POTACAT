...
const RIG_CONTROLS = {
  ...
  // Preamp/ATT accept a LADDER STEP as well as a boolean: when the rig
  // advertises caps.preampSteps / caps.attSteps (IPO/AMP1/AMP2, 6/12/18 dB,
  // or hamlib's probed dB list — see lib/rig-gain-steps.js), `value` may be a
  // step value and the client should render a cycling control. `value: true`
  // still means "first ON step", so a client that only knows the toggle keeps
  // working — that back-compat is why these stay kind:'toggle'. State comes
  // back as preamp/att (boolean) plus preampStep/attStep (the position).
  'set-preamp':        { kind: 'toggle', group: 'rx', caps: 'preamp', stepsCaps: 'preampSteps', label: 'Preamp', help: 'Receive preamplifier (cycles IPO/AMP1/AMP2 on rigs with steps)' },
  'set-att':           { kind: 'toggle', group: 'rx', caps: 'att', stepsCaps: 'attSteps', label: 'Att', help: 'Receiver attenuator (cycles 6/12/18 dB on rigs with steps)' },
  ...
  // --- VFO / split ---
  'set-vfo':           { kind: 'enum',   group: 'freq', caps: 'vfo', label: 'VFO', help: 'Active VFO (A/B)' },
  'swap-vfo':          { kind: 'momentary', group: 'freq', caps: 'vfo', label: 'A/B', help: 'Switch between VFO A and VFO B' },
  'set-split':         { kind: 'toggle', group: 'freq', caps: 'split', txOnly: true, label: 'Split', help: 'Split operation — transmit on the other VFO' },
