'use strict';
//
// ECHOCAT WebSocket Protocol — schemas of record.
//
// This module is the single source of truth for the message types that
// flow between POTACAT desktop (server) and an ECHOCAT client (browser
// today, native mobile app coming).
//
// Constraints:
//  - Pure JavaScript. No DOM. No Node-only built-ins. Must work in
//    React Native, in the desktop main process, and in a Node CLI.
//  - Tiny: no schema-validation dep. We hand-roll a small validator
//    so the bundle stays small and the wire format stays explicit.
//  - Backward compatible with the existing browser ECHOCAT, which
//    pre-dates this module and does not send a `hello`.
//
// See docs/echocat-protocol.md for the human-readable catalog.
//

/** Current protocol major version. Bump when the wire format breaks. */
const PROTOCOL_VERSION = 1;

/**
 * WebSocket close codes. Application range is 4000-4999.
 *  - 4001 unsupported version: peer is too far ahead/behind to talk.
 *  - 4002 bad handshake: malformed `hello`.
 *  - 4003 auth failed terminally: stop reconnecting (client decides).
 *  - 4004 auth revoked: the operator revoked this device's pairing while
 *    it was connected. Preceded by a `revoked` message. Don't reconnect —
 *    the device token is gone; a reconnect would only hit a terminal
 *    auth-fail (the server can no longer distinguish revoked from
 *    never-paired once the record is deleted).
 * Mirror of mobile's src/protocol/echocatProtocol.ts — keep in sync.
 */
const CLOSE_CODES = Object.freeze({
  PROTOCOL_VERSION_UNSUPPORTED: 4001,
  HANDSHAKE_INVALID: 4002,
  AUTH_FAILED_TERMINAL: 4003,
  AUTH_REVOKED: 4004,
});

/**
 * Direction enum.
 *  - s2c: server → client only
 *  - c2s: client → server only
 *  - both: either direction
 */
const Dir = Object.freeze({ S2C: 's2c', C2S: 'c2s', BOTH: 'both' });

/**
 * Field-shape primitives the validator understands. Keep this small —
 * we want explicit shapes, not a general-purpose JSON schema.
 *  - 'string' | 'number' | 'integer' | 'boolean' | 'object' | 'array'
 *  - 'any' for fields whose shape is too dynamic to be worth pinning
 *  - { oneOf: [...] } for enum-of-strings
 *
 * `required: true` is the default. Set to `false` for optional fields.
 */

// Field shorthand used below to keep the registry readable.
const f = {
  string: { type: 'string' },
  optString: { type: 'string', required: false },
  number: { type: 'number' },
  optNumber: { type: 'number', required: false },
  integer: { type: 'integer' },
  optInteger: { type: 'integer', required: false },
  boolean: { type: 'boolean' },
  optBoolean: { type: 'boolean', required: false },
  array: { type: 'array' },
  optArray: { type: 'array', required: false },
  object: { type: 'object' },
  optObject: { type: 'object', required: false },
  any: { type: 'any', required: false },
  oneOf: (values) => ({ type: 'oneOf', values, required: true }),
  optOneOf: (values) => ({ type: 'oneOf', values, required: false }),
};

/**
 * Message registry.
 *
 * Keyed by `type` field. Each entry declares:
 *   - dir: which direction(s) it flows
 *   - fields: declared field shapes (extra fields allowed but undocumented)
 *   - feature: tag used by the catalog doc
 *
 * NOT every existing message has a fully-typed schema — many older
 * messages use `_` (any-bag) so the v1 cutover is non-breaking. Tighten
 * fields incrementally as features are touched.
 */
const MESSAGES = Object.freeze({
  // ─── Handshake / connection ─────────────────────────────────────────
  hello: {
    dir: Dir.BOTH,
    feature: 'handshake',
    fields: {
      protocolVersion: f.integer,
      // Server-side fields
      serverVersion: f.optString,
      capabilities: f.optArray,
      // Active rig model on the shack ("Flex 8600M", "FTDX10", "IC-7300", …).
      // Lets POTACAT desktop clients in the Remote Radios panel distinguish
      // between multiple paired shacks. Empty string when no rig configured.
      rigModel: f.optString,
      // Client-side fields
      clientVersion: f.optString,
      clientPlatform: f.optString,
    },
  },
  'auth-mode': { dir: Dir.S2C, feature: 'handshake', fields: { mode: f.string } },
  auth: { dir: Dir.C2S, feature: 'handshake', fields: { token: f.optString, callsign: f.optString, password: f.optString } },
  // auth-ok carries the bulk of the initial state (settings, feature flags, alt hosts).
  // expiresAt/accountLinked/trusted describe the paired-device row that just authenticated:
  //   - expiresAt: epoch ms when the deviceToken expires, or null/undefined for no-expiry
  //     (trusted devices, account-linked devices). Client uses this to drive the T-14d
  //     re-pair nudge and the "expires in N days" badge in Remote Radios.
  //   - accountLinked: pair came in via Cloud-account attestation (Path 1 in the
  //     desktop-to-desktop plan). Never expires; cloud-revocable.
  //   - trusted: shack operator manually flagged this device as their own. Never expires.
  // For the legacy single-shared-token path and Guest Pass auth, these are absent.
  // tsHost/cloudHost/tsCertPublic/fingerprint are the shack's CURRENT alternate
  // dials + trust info, refreshed on every connect — a client paired cloud-only
  // should ADOPT tsHost from here instead of considering the pairing frozen
  // (LZ3AW 2026-08-03: these were sent since v1.8.5 but unregistered, so the
  // mobile mirror never knew they existed — the tx-meter lesson again).
  //   - tsCertPublic: true = the served cert is a Tailscale-issued LE cert that
  //     validates publicly; dial tsHost with standard TLS, no pin. Absent/false
  //     = self-signed → pin `fingerprint`.
  //   - fingerprint: SHA-256 (colon-hex) of the cert this server is presenting
  //     NOW — also the recovery path for a rotated/renewed cert without re-pairing.
  'auth-ok': { dir: Dir.S2C, feature: 'handshake', fields: { settings: f.optObject, expiresAt: f.optNumber, accountLinked: f.optBoolean, trusted: f.optBoolean, tsHost: f.optString, cloudHost: f.optString, tsCertPublic: f.optBoolean, fingerprint: f.optString, colorblindMode: f.optBoolean, cwAvailable: f.optBoolean, cwPaddleAvailable: f.optBoolean, cwPaddleReason: f.optString, vfoLocked: f.optBoolean, stationCallsign: f.optString } },
  // Mid-session update of the same alternate-dial info auth-ok carries —
  // pushed whenever tsHost/cloudHost actually change (operator signs into
  // Tailscale, Cloud Tunnel comes up/down) so a connected client learns the
  // new dial without reconnecting. Sent since v1.8.5; registered 2026-08-03.
  'alt-hosts': { dir: Dir.S2C, feature: 'handshake', fields: { tsHost: f.optString, cloudHost: f.optString, tsCertPublic: f.optBoolean, fingerprint: f.optString } },
  'auth-fail': { dir: Dir.S2C, feature: 'handshake', fields: { reason: f.string } },
  kicked: { dir: Dir.S2C, feature: 'handshake', fields: { reason: f.string } },
  // The operator revoked this device's pairing while it was connected.
  // Sent immediately before close(4004 AUTH_REVOKED). Unlike `kicked`
  // (another device took over — reconnecting later is fine), revoked
  // means the device token no longer exists: drop to the paired-out
  // state and don't auto-reconnect.
  revoked: { dir: Dir.S2C, feature: 'handshake', fields: { reason: f.string } },
  ping: { dir: Dir.C2S, feature: 'handshake', fields: { ts: f.optNumber } },
  pong: { dir: Dir.S2C, feature: 'handshake', fields: { ts: f.optNumber } },

  // ─── Spots / sources ────────────────────────────────────────────────
  spots: { dir: Dir.S2C, feature: 'spots', fields: { data: f.array } },
  // Desktop-to-desktop (remote-desktop-plan, Phase 1). Same payload as
  // `spots` (full merged array); a distinct type so a desktop client can
  // tell a throttled refresh from the initial auth-ok push. RemoteClient
  // already handles it (lib/remote-client.js) — this registry entry makes
  // it encodable/validatable too (it was handled-but-unregistered).
  'spots:update': { dir: Dir.S2C, feature: 'spots', fields: { data: f.array } },
  sources: { dir: Dir.S2C, feature: 'spots', fields: { data: f.object } },
  'set-sources': { dir: Dir.C2S, feature: 'spots', fields: { sources: f.object } },
  // echo-filters `data` carries the client's own roaming filter prefs PLUS
  // `muteRules` ([{continent:'AS', band:'40m'}] — desktop-owned per-band
  // region mutes, merged in at send; N7BBQ 2026-08-03). Clients apply
  // muteRules in their spot filter and MUST display active rules.
  'echo-filters': { dir: Dir.S2C, feature: 'spots', fields: { data: f.object } },
  'set-echo-filters': { dir: Dir.C2S, feature: 'spots', fields: { filters: f.object } },
  // Full-list replace of the mute rules from a client editor; desktop
  // sanitizes and echoes the accepted set inside the next echo-filters push.
  'set-spot-mute-rules': { dir: Dir.C2S, feature: 'spots', fields: { rules: f.array } },
  'worked-parks': { dir: Dir.S2C, feature: 'spots', fields: { refs: f.array } },
  'worked-qsos': { dir: Dir.S2C, feature: 'spots', fields: { entries: f.array } },
  // ─── Special events (13 Colonies, ARRL 250, …) ──────────────────────
  // Catalog + subscriptions + merged per-event checklist progress ride the
  // settings blob (availableEvents / eventSubscriptions inside settings-update /
  // auth-ok). These two are the phone→desktop mutations. set-event-progress
  // manually checks/unchecks a checklist station for QSOs logged off-app.
  // Phone-initiated event-catalog refetch ("re-crawl") — the desktop polls
  // potacat.com/events/active.json every 4 h; this forces the fetch NOW so a
  // just-published event (WRTC went live 5 min before its contest) reaches
  // the shack without waiting out the cycle. Ack means "refetch started";
  // the updated catalog arrives via the normal settings-update broadcast
  // (availableEvents / contestCatalogExtras). Desktop rate-limits to one
  // per 10 s — extra taps are silently dropped.
  'refresh-events': { dir: Dir.C2S, feature: 'spots' },
  'refresh-events-ack': { dir: Dir.S2C, feature: 'spots' },
  'set-event-subscription': { dir: Dir.C2S, feature: 'spots', fields: { eventId: f.string, tracking: f.optBoolean, mutedPhone: f.optBoolean, mutedDesktop: f.optBoolean } },
  'set-event-progress': { dir: Dir.C2S, feature: 'spots', fields: { eventId: f.string, itemId: f.string, worked: f.boolean } },
  // Desktop-to-desktop (remote-desktop-plan, Phase 1). Watchlist flows BOTH
  // ways and is last-write-wins: a laptop client edits its watchlist, the
  // shack adopts it and re-broadcasts so any other display stays in sync.
  // The colon-namespaced names match the plan's cross-repo contract table.
  'watchlist:sync': { dir: Dir.BOTH, feature: 'spots', fields: { callsigns: f.array, groups: f.optArray } },

  // ─── Rig control / VFO ──────────────────────────────────────────────
  status: { dir: Dir.S2C, feature: 'rig' },
  // tune: phone sends `freqKhz` as a STRING (server parses it as a float
  // kHz value). Earlier the schema lied and declared `frequency: number` —
  // fixed when the mobile-app dev hit it. Optional `bearing` rotates an
  // antenna rotor for the spot's bearing. (Gap 5, 2026-05-03.)
  tune: { dir: Dir.C2S, feature: 'rig', fields: { freqKhz: f.string, mode: f.optString, bearing: f.optNumber } },
  'tune-blocked': { dir: Dir.S2C, feature: 'rig', fields: { reason: f.string } },
  'set-mode': { dir: Dir.C2S, feature: 'rig', fields: { mode: f.string } },
  'set-vfo': { dir: Dir.C2S, feature: 'rig', fields: { vfo: f.string } },
  'swap-vfo': { dir: Dir.C2S, feature: 'rig' },
  'set-filter': { dir: Dir.C2S, feature: 'rig', fields: { width: f.number } },
  'filter-step': { dir: Dir.C2S, feature: 'rig', fields: { dir: f.string } },
  'set-rfgain': { dir: Dir.C2S, feature: 'rig', fields: { value: f.number } },
  'set-txpower': { dir: Dir.C2S, feature: 'rig', fields: { value: f.number } },
  'set-nb': { dir: Dir.C2S, feature: 'rig', fields: { on: f.boolean } },
  'set-atu': { dir: Dir.C2S, feature: 'rig', fields: { on: f.boolean } },
  // Both handlers destructure `value` — the registry said `on` for months, so
  // a client following the registry was silently ignored (found during the
  // LZ3AW VFO work, 2026-08-03). Registry now matches the shipped handlers.
  // These toggle the spot-tune SETTINGS (use ATU / use split on spot tunes);
  // live rig split is `rig-control {action:'set-split', value}`.
  'set-enable-atu': { dir: Dir.C2S, feature: 'rig', fields: { value: f.boolean } },
  'set-enable-split': { dir: Dir.C2S, feature: 'rig', fields: { value: f.boolean } },
  'set-cw-xit': { dir: Dir.C2S, feature: 'rig', fields: { hz: f.number } },
  'set-cw-filter': { dir: Dir.C2S, feature: 'rig', fields: { width: f.number } },
  'set-ssb-filter': { dir: Dir.C2S, feature: 'rig', fields: { width: f.number } },
  'set-digital-filter': { dir: Dir.C2S, feature: 'rig', fields: { width: f.number } },
  'set-tune-click': { dir: Dir.C2S, feature: 'rig', fields: { mode: f.string } },
  'set-scan-dwell': { dir: Dir.C2S, feature: 'rig', fields: { seconds: f.number } },
  'set-max-age': { dir: Dir.C2S, feature: 'rig', fields: { minutes: f.number } },
  'set-dist-unit': { dir: Dir.C2S, feature: 'rig', fields: { unit: f.string } },
  'set-refresh-interval': { dir: Dir.C2S, feature: 'rig', fields: { seconds: f.number } },
  'scan-step': { dir: Dir.C2S, feature: 'rig', fields: { action: f.string } },
  // Desktop-to-desktop (remote-desktop-plan, Phase 1). The legacy `scan-step`
  // only nudges; a remote client needs the shack to PUSH scan state and to
  // start/stop/skip the scan. scan:control is the command, scan:state the
  // pushed status. Colon-namespaced to match the plan's contract table.
  'scan:control': { dir: Dir.C2S, feature: 'rig', fields: { action: f.oneOf(['start', 'stop', 'skip', 'unskip']), spotId: f.optString } },
  'scan:state': { dir: Dir.S2C, feature: 'rig', fields: { running: f.boolean, currentRef: f.optString, dwellRemaining: f.optNumber, skip: f.optArray } },
  // scan on/off STATE SYNC between desktop and mobile (scan-state-sync-desktop).
  // These are the canonical scan-sync messages mobile shipped against — both
  // Dir.BOTH so either side announces its own engine's on/off and can ask the
  // peer to stop. NOTE: the `scan:*` (colon) pair just above is vestigial —
  // registered but wired nowhere — superseded by these. Kept tolerant: the
  // registry throws on shape mismatch, so `scanning` stays boolean / `action`
  // string. Gated like other rig C2S (authenticated active client; not
  // owner-only — same posture as tune/ptt today). (Casey 2026-06-16)
  'scan-state': { dir: Dir.BOTH, feature: 'rig', fields: { scanning: f.boolean } },
  'scan-control': { dir: Dir.BOTH, feature: 'rig', fields: { action: f.string } },
  // Canonical shape: {type:'rig-control', data:{action:'set-split', value:true}}
  // — the demux reads msg.data.action (top-level action accepted as a
  // fallback since 2026-08-03, but nest under data; the first handoff doc
  // got this wrong and the mobile dev caught it against the shipped senders).
  'rig-control': { dir: Dir.C2S, feature: 'rig', fields: { data: f.optObject } },
  'rig-blocked': { dir: Dir.S2C, feature: 'rig', fields: { reason: f.string } },
  rigs: { dir: Dir.S2C, feature: 'rig', fields: { data: f.array, activeRigId: f.optString } },
  'switch-rig': { dir: Dir.C2S, feature: 'rig', fields: { rigId: f.string } },
  'tgxl-select-antenna': { dir: Dir.C2S, feature: 'rig', fields: { antenna: f.optString } },
  'toggle-rotor': { dir: Dir.C2S, feature: 'rig' },
  'vfo-set-lock': { dir: Dir.C2S, feature: 'rig', fields: { locked: f.boolean } },
  'vfo-lock-state': { dir: Dir.S2C, feature: 'rig', fields: { locked: f.boolean } },
  'vfo-profiles': { dir: Dir.BOTH, feature: 'rig', fields: { profiles: f.optArray } },
  'vfo-profiles-update': { dir: Dir.C2S, feature: 'rig', fields: { profiles: f.array } },
  // Payload is the WHOLE profile object (name/freqKhz/mode/filterWidth),
  // NOT an id — the desktop handler (main.js on('apply-vfo-profile'))
  // reads `msg.profile` and tunes to it; there are no stable profile ids
  // (identity is list position). The old `{ id: f.string }` was stale
  // metadata: validate() only runs on `hello` here so it never bit the
  // desktop, but the mobile mirror validates every outbound C2S via
  // encode(), where a required-but-absent `id` made a valid apply throw.
  // Keep in sync with mobile src/protocol/echocatProtocol.ts. (K3SBP.)
  'apply-vfo-profile': { dir: Dir.C2S, feature: 'rig', fields: { profile: f.object } },
  'settings-update': { dir: Dir.S2C, feature: 'rig', fields: { settings: f.object } },
  'save-settings': { dir: Dir.C2S, feature: 'rig', fields: { settings: f.object } },

  // ─── PTT / WebRTC signaling ─────────────────────────────────────────
  ptt: { dir: Dir.C2S, feature: 'ptt', fields: { state: f.boolean } },
  estop: { dir: Dir.C2S, feature: 'ptt' },
  'ptt-timeout': { dir: Dir.S2C, feature: 'ptt' },
  'ptt-force-rx': { dir: Dir.S2C, feature: 'ptt' },
  'start-audio': { dir: Dir.C2S, feature: 'ptt' },
  signal: { dir: Dir.BOTH, feature: 'ptt', fields: { data: f.any } },
  // stun-config: shack → client ICE configuration for the audio WebRTC peer.
  // Legacy `useStun` bool, plus — when the shack mints a Cloudflare TURN
  // relay (Model A, cloud-audio-turn-relay) — the full `iceServers` array and
  // remaining `iceTtlMs`. `relayOnly` forces iceTransportPolicy:'relay' for
  // testing. Consumed by the web client (renderer/remote.js), the iOS app,
  // and the desktop-client answerer (remote-desktop-plan Phase 2). It was
  // sent-but-unregistered (_sendTo doesn't validate); registering it
  // completes the contract and lets the test cover it.
  'stun-config': { dir: Dir.S2C, feature: 'ptt', fields: { useStun: f.optBoolean, iceServers: f.optArray, iceTtlMs: f.optNumber, relayOnly: f.optBoolean } },
  // Legacy WebRTC envelopes — pre-`signal` clients still send these.
  // Server keeps accepting them; new clients should not.
  sdp: { dir: Dir.C2S, feature: 'ptt-legacy', fields: { sdp: f.any, sessionDescription: f.any } },
  ice: { dir: Dir.C2S, feature: 'ptt-legacy', fields: { candidate: f.any } },
  'get-audio-devices': { dir: Dir.C2S, feature: 'ptt' },
  'set-audio-device': { dir: Dir.C2S, feature: 'ptt', fields: { kind: f.string, deviceId: f.string } },
  // TX DRIVE — the SHACK-side level of the client-mic audio POTACAT feeds the
  // radio (dax_tx / USB CODEC). `value` is an absolute percent 0–200 (100 =
  // unity). This is NOT a client-local mic gain: the web client's own
  // #rc-tx-gain slider scales that browser's microphone before WebRTC and
  // never leaves the browser. Reserve the name "mic level" for any future
  // client-local gain so the two can't be confused (tx-audio-level-mobile ask).
  // Desktop-authoritative: a set is clamped + persisted per-rig and echoed to
  // every client as `tx-drive-state`, which also hydrates at connect — so a
  // client never renders a stale slider. Gated: Guest Pass clients are refused
  // (they must not move the host's TX drive). Capability: 'tx-drive'.
  'set-tx-drive': { dir: Dir.C2S, feature: 'ptt', fields: { value: f.number } },
  'tx-drive-state': { dir: Dir.S2C, feature: 'ptt', fields: { value: f.number } },
  // PC-side TX peak (0..1 float), pushed ~30ms while the audio bridge is up so
  // a client can render the same meter the desktop shows next to the TX drive
  // slider. Existed and was handled by the web client long before it was
  // registered here (handled-but-unregistered); registered 2026-08-01 so the
  // mobile mirror can consume it.
  'tx-meter': { dir: Dir.S2C, feature: 'ptt', fields: { value: f.number } },

  // ─── Activator mode ─────────────────────────────────────────────────
  'activator-state': { dir: Dir.S2C, feature: 'activator' },
  'set-activator-park': { dir: Dir.C2S, feature: 'activator', fields: { parkRefs: f.optArray, activationName: f.optString, sig: f.optString, activationType: f.optString } },
  'session-contacts': { dir: Dir.S2C, feature: 'activator', fields: { contacts: f.array } },

  // ─── Logging ────────────────────────────────────────────────────────
  // Payload is a legacy any-bag; two fields worth naming (2026-08-09):
  // qsoAt (epoch ms) = the EXCHANGE time — honored over the tap time, with
  // now as the fallback so older clients are unchanged; theirGrid = the
  // worked station's grid → ADIF GRIDSQUARE (air-heard beats park beats
  // QRZ-home precedence on the desktop).
  'log-qso': { dir: Dir.C2S, feature: 'logging' },
  'log-ok': { dir: Dir.S2C, feature: 'logging' },
  // Daily QSO milestone crossed (10/25/50/100/150/200/250/500 per UTC day,
  // active operator's logbook) — fires once per milestone per UTC day. The
  // phone renders its native celebration (mobile handoff: qso-milestone-
  // celebrations): cat pop for regular milestones, full-screen mega at 500.
  'qso-milestone': { dir: Dir.S2C, feature: 'logging', fields: { milestone: f.number, count: f.number, mega: f.boolean, message: f.string } },
  // Architecture B (v1.9, Brief C): host forwards an auto-logged QSO
  // (WSJT-X bridge / JTCAT engine) to the active client so the client
  // can write it to ITS logbook + cloud-sync under its own JWT.
  // Client must advertise capabilities:['qso-attributed'] in hello to
  // receive these — otherwise the host emits log-error per the hard
  // rule (a guest's log never lands in the owner's logbook).
  'qso-attributed': { dir: Dir.S2C, feature: 'logging', fields: { qso: f.object } },
  // Architecture B: host couldn't deliver a forwarded QSO to the
  // client. The client surfaces a verbose modal (NOT a toast) with
  // the full QSO details so the operator can write the contact down
  // by hand. See brief-b-additions.md §3 for the modal copy spec.
  // reason: 'no-capability' | 'forward-failed' | 'ws-dropped' | other.
  'log-error': { dir: Dir.S2C, feature: 'logging', fields: { qso: f.object, reason: f.string, message: f.optString } },
  'get-all-qsos': { dir: Dir.C2S, feature: 'logging' },
  // data is one slice when chunked (chunk/totalChunks/total present), else the
  // full list. `truncated` flags a legacy client's most-recent-N cap. All extra
  // fields optional so the single-frame path still validates. (all-qsos-chunking)
  'all-qsos': { dir: Dir.S2C, feature: 'logging', fields: { data: f.array, chunk: f.optNumber, totalChunks: f.optNumber, total: f.optNumber, truncated: f.optBoolean } },
  // Incremental append pushed after each QSO save to clients whose hello
  // advertises 'qso-delta', instead of re-pushing the whole log (the legacy
  // single-frame re-push was the 9.6MB iOS 1009 killer, BUG-N3VD-20260701).
  // `data` is one record in all-qsos shape (idx = position in the full log),
  // `total` the new log length so the client can detect a missed delta and
  // fall back to get-all-qsos.
  'qso-added': { dir: Dir.S2C, feature: 'logging', fields: { data: f.object, total: f.optNumber } },
  'update-qso': { dir: Dir.C2S, feature: 'logging', fields: { idx: f.integer, fields: f.object } },
  'qso-updated': { dir: Dir.S2C, feature: 'logging' },
  'delete-qso': { dir: Dir.C2S, feature: 'logging', fields: { idx: f.integer } },
  'qso-deleted': { dir: Dir.S2C, feature: 'logging' },
  'lookup-call': { dir: Dir.C2S, feature: 'logging', fields: { call: f.string } },
  'qrz-lookup': { dir: Dir.C2S, feature: 'logging', fields: { call: f.string } },
  'call-lookup': { dir: Dir.S2C, feature: 'logging' },
  'search-parks': { dir: Dir.C2S, feature: 'logging', fields: { query: f.string } },
  'park-results': { dir: Dir.S2C, feature: 'logging', fields: { results: f.array } },
  // "Parks near me": distance-sorted parks around a point, for one-tap
  // activation-start selection on mobile (activator-mode-plan.md Phase 2
  // items 8-9). Results carry distanceMi + bearingDeg per park.
  'nearby-parks': { dir: Dir.C2S, feature: 'logging', fields: { lat: f.number, lon: f.number, limit: f.optNumber } },
  'nearby-park-results': { dir: Dir.S2C, feature: 'logging', fields: { results: f.array } },
  'get-past-activations': { dir: Dir.C2S, feature: 'logging', fields: { ref: f.string } },
  'past-activations': { dir: Dir.S2C, feature: 'logging', fields: { data: f.array } },
  'get-activation-map-data': { dir: Dir.C2S, feature: 'logging' },
  'activation-map-data': { dir: Dir.S2C, feature: 'logging' },

  // ─── Directory / donors ─────────────────────────────────────────────
  directory: { dir: Dir.S2C, feature: 'directory', fields: { nets: f.optArray, swl: f.optArray } },
  'donor-callsigns': { dir: Dir.S2C, feature: 'directory', fields: { callsigns: f.array } },

  // ─── JTCAT (FT8 / FT4 / FT2) ────────────────────────────────────────
  'jtcat-start': { dir: Dir.C2S, feature: 'jtcat', fields: { mode: f.optString } },
  'jtcat-stop': { dir: Dir.C2S, feature: 'jtcat' },
  'jtcat-status': { dir: Dir.S2C, feature: 'jtcat' },
  'jtcat-set-mode': { dir: Dir.C2S, feature: 'jtcat', fields: { mode: f.string } },
  'jtcat-set-band': { dir: Dir.C2S, feature: 'jtcat', fields: { band: f.string } },
  'jtcat-set-tx-freq': { dir: Dir.C2S, feature: 'jtcat', fields: { hz: f.number } },
  'jtcat-set-tx-slot': { dir: Dir.C2S, feature: 'jtcat', fields: { slot: f.string } },
  // Gain sliders. The wire field is `value` (0–1 float) — every sender (web
  // client, mobile, remote-server demux) has always used `value`; the
  // registry said `level` and never matched reality (fixed 2026-07-20).
  // RX gain is DESKTOP-AUTHORITATIVE and synced across all surfaces (main
  // window, popout, clients): a C2S set is applied + persisted
  // (settings.jtcatRxGain) and echoed to clients as jtcat-rx-gain-state,
  // which also hydrates at connect — so a client can SEE a zeroed RX slider
  // (the 2026-07-18 blank-waterfall cause) instead of guessing.
  'jtcat-rx-gain': { dir: Dir.C2S, feature: 'jtcat', fields: { value: f.number } },
  'jtcat-tx-gain': { dir: Dir.C2S, feature: 'jtcat', fields: { value: f.number } },
  'jtcat-rx-gain-state': { dir: Dir.S2C, feature: 'jtcat', fields: { value: f.number } },
  'jtcat-enable-tx': { dir: Dir.C2S, feature: 'jtcat', fields: { enabled: f.boolean } },
  'jtcat-halt-tx': { dir: Dir.C2S, feature: 'jtcat' },
  'jtcat-call-cq': { dir: Dir.C2S, feature: 'jtcat', fields: { modifier: f.optString } },
  // jtcat-reply carries the phone's full tap intent. `text` (raw decode) is
  // the load-bearing field: the host re-derives the QSO step from it
  // authoritatively, so a stale client classifier can't send the wrong
  // opener. nextStep/theirGrid/theirReport are the client's own
  // classification; grid/report/rr73 are legacy equivalents older apps send.
  // (2026-07-09: the server demux used to strip everything but
  // call/grid/df/sliceId — every phone tap on a directed decode answered
  // with a grid instead of a report. See test/echocat-protocol-test.js.)
  'jtcat-reply': { dir: Dir.C2S, feature: 'jtcat', fields: {
    call: f.string, grid: f.optString, df: f.optNumber, sliceId: f.optString,
    text: f.optString, nextStep: f.optString, snr: f.optNumber, slot: f.optString,
    theirGrid: f.optString, theirReport: f.optString, rr73: f.optBoolean, report: f.optString,
  } },
  'jtcat-cancel-qso': { dir: Dir.C2S, feature: 'jtcat' },
  'jtcat-skip-phase': { dir: Dir.C2S, feature: 'jtcat' },
  'jtcat-log-qso': { dir: Dir.C2S, feature: 'jtcat' },
  'jtcat-auto-cq-mode': { dir: Dir.C2S, feature: 'jtcat', fields: { mode: f.string } },
  'jtcat-auto-cq-state': { dir: Dir.S2C, feature: 'jtcat' },
  // Chase target — the CQ tag / entity the operator is chasing (drives the
  // outgoing CQ tag and the incoming decode highlight). Shared preference,
  // last-writer-wins. Phone sets via C2S; desktop echoes the agreed value S2C.
  'jtcat-set-chase-target': { dir: Dir.C2S, feature: 'jtcat', fields: { tag: f.optString } },
  'jtcat-chase-target': { dir: Dir.S2C, feature: 'jtcat', fields: { tag: f.optString } },
  // ULTRACAT (hidden tier-2). Tells the client whether the desktop is
  // ULTRACAT-unlocked and whether Full Auto CQ run mode is active, so the
  // mobile app can reveal + mirror the matching controls.
  'jtcat-ultracat-state': { dir: Dir.S2C, feature: 'jtcat' },
  // Phone drives Full Auto CQ run mode (the §5 control path of
  // ios-handoffs/ultracat-full-auto-cq). Desktop-side startFullAutoCq guards
  // the ULTRACAT unlock — a locked desktop refuses and re-broadcasts
  // jtcat-ultracat-state so the phone's toggle reconciles. `on:false` always
  // stops, regardless of who started the run (any client may silence TX).
  'jtcat-full-auto-cq': { dir: Dir.C2S, feature: 'jtcat', fields: { on: f.boolean, modifier: f.optString } },
  // Spot Target (docs/mobile-handoff-spot-target.md): tap a digital-mode spot,
  // the desktop watches decodes for that call and auto-calls at a polite
  // opening (their CQ / QSO end). `target` is sanitized desktop-side on
  // ingest — never acted on verbatim. NOTE: target.freqKhz is a NUMBER (kHz),
  // unlike tune.freqKhz which is a STRING — same name, two types, both
  // registries carry this warning. Gated by the 'spot-target' hello
  // capability so older desktops never receive a tap they'd silently drop.
  'jtcat-spot-target-set': { dir: Dir.C2S, feature: 'jtcat', fields: { target: f.object } },
  'jtcat-spot-target-clear': { dir: Dir.C2S, feature: 'jtcat' },
  'jtcat-spot-target-call-now': { dir: Dir.C2S, feature: 'jtcat' },
  // S2C state mirror: armed/engaged/cleared + heard refreshes + connect
  // hydration (hydration is ADOPTION — the phone toasts only on live
  // transitions, never the first push of a session).
  'jtcat-spot-target': { dir: Dir.S2C, feature: 'jtcat' },
  'jtcat-decode': { dir: Dir.S2C, feature: 'jtcat' },
  'jtcat-decode-batch': { dir: Dir.S2C, feature: 'jtcat', fields: { entries: f.array } },
  // WSPR (beacon/propagation mode). Spots flow host->client; the client may
  // drive the beacon — the host owns the 1 W power cap, the attended-operator
  // watchdog, and the TX path, so the client only sends intent.
  'jtcat-wspr-spots': { dir: Dir.S2C, feature: 'jtcat', fields: { spots: f.array, error: f.optString } },
  'jtcat-wspr-beacon': { dir: Dir.C2S, feature: 'jtcat', fields: { enabled: f.optBoolean, txPct: f.optNumber, dBm: f.optNumber } },
  'jtcat-wspr-beacon-state': { dir: Dir.S2C, feature: 'jtcat', fields: { enabled: f.boolean } },
  'jtcat-cycle': { dir: Dir.S2C, feature: 'jtcat' },
  'jtcat-tx-status': { dir: Dir.S2C, feature: 'jtcat' },
  'jtcat-qso-state': { dir: Dir.S2C, feature: 'jtcat' },
  'jtcat-spectrum': { dir: Dir.S2C, feature: 'jtcat', fields: { bins: f.array } },
  // Mobile gates the desktop's in-process spectrum FFT on this
  // subscribe. Sent { on: true } when the user opens the spectrum
  // panel; { on: false } when it's closed or the WS reconnects.
  // Desktop only runs the FFT loop when at least one consumer is
  // listening, so CPU is zero when nobody's looking. K3SBP 2026-05-31.
  'jtcat-spectrum-subscribe': { dir: Dir.C2S, feature: 'jtcat', fields: { on: f.boolean } },
  'jtcat-waterfall': { dir: Dir.C2S, feature: 'jtcat', fields: { enabled: f.boolean } },
  'jtcat-start-multi-remote': { dir: Dir.C2S, feature: 'jtcat' },
  // WSJT-X-style steady-tone Tune. Phone tap toggles; desktop emits state
  // back so the phone UI can light up the button + show the countdown.
  // (Gap 11, mobile dev report 2026-05-04.)
  'jtcat-tune-toggle': { dir: Dir.C2S, feature: 'jtcat' },
  'jtcat-tune-state': { dir: Dir.S2C, feature: 'jtcat', fields: { active: f.boolean, secondsRemaining: f.optInteger } },
  // Auto Seq: when true, the QSO state machine auto-advances on each
  // matching decode. When false, engine still records what it heard but
  // doesn't compose the next TX — user drives via Skip. (Gap 12.)
  'jtcat-set-auto-seq': { dir: Dir.C2S, feature: 'jtcat', fields: { enabled: f.boolean } },
  'jtcat-auto-seq-state': { dir: Dir.S2C, feature: 'jtcat', fields: { enabled: f.boolean } },

  // ─── FreeDV ─────────────────────────────────────────────────────────
  'freedv-start': { dir: Dir.C2S, feature: 'freedv', fields: { mode: f.optString } },
  'freedv-stop': { dir: Dir.C2S, feature: 'freedv' },
  'freedv-set-mode': { dir: Dir.C2S, feature: 'freedv', fields: { mode: f.string } },
  'freedv-set-tx': { dir: Dir.C2S, feature: 'freedv', fields: { on: f.boolean } },
  'freedv-set-squelch': { dir: Dir.C2S, feature: 'freedv', fields: { level: f.number } },
  'set-freedv': { dir: Dir.C2S, feature: 'freedv', fields: { enabled: f.boolean } },
  // freedv-enabled: server tells client whether the FreeDV master toggle is
  // on. Pushed at startup and whenever settings.enableFreedv changes. (Gap 1.)
  'freedv-enabled': { dir: Dir.S2C, feature: 'freedv', fields: { enabled: f.boolean } },

  // ─── CW ─────────────────────────────────────────────────────────────
  'cw-available': { dir: Dir.S2C, feature: 'cw', fields: { enabled: f.boolean } },
  'cw-paddle-available': { dir: Dir.S2C, feature: 'cw' },
  'cw-config': { dir: Dir.C2S, feature: 'cw', fields: { wpm: f.optNumber, mode: f.optString } },
  'cw-config-ack': { dir: Dir.S2C, feature: 'cw', fields: { wpm: f.number, mode: f.string } },
  'cw-state': { dir: Dir.S2C, feature: 'cw', fields: { keying: f.boolean } },
  'cw-text': { dir: Dir.C2S, feature: 'cw', fields: { text: f.string, wpm: f.optNumber } },
  'cw-stop': { dir: Dir.C2S, feature: 'cw' },
  'cw-enable': { dir: Dir.C2S, feature: 'cw', fields: { enabled: f.boolean } },
  paddle: { dir: Dir.C2S, feature: 'cw', fields: { event: f.string } },
  'save-cw-macros': { dir: Dir.C2S, feature: 'cw', fields: { macros: f.array } },

  // ─── SSTV ───────────────────────────────────────────────────────────
  'sstv-open': { dir: Dir.C2S, feature: 'sstv' },
  'sstv-photo': { dir: Dir.C2S, feature: 'sstv' },
  'sstv-stop': { dir: Dir.C2S, feature: 'sstv' },
  'sstv-halt-tx': { dir: Dir.C2S, feature: 'sstv' },
  'sstv-get-gallery': { dir: Dir.C2S, feature: 'sstv' },
  'sstv-gallery': { dir: Dir.S2C, feature: 'sstv' },
  'sstv-get-compose': { dir: Dir.C2S, feature: 'sstv' },
  'sstv-compose-state': { dir: Dir.S2C, feature: 'sstv' },
  'sstv-rx-image': { dir: Dir.S2C, feature: 'sstv' },
  'sstv-rx-progress': { dir: Dir.S2C, feature: 'sstv' },
  'sstv-tx-status': { dir: Dir.S2C, feature: 'sstv' },
  'sstv-wf-bins': { dir: Dir.S2C, feature: 'sstv', fields: { bins: f.array } },
  // Mobile toggles auto-SSTV-on-idle remotely. Banner appears on the phone
  // when desktop pushes `sstv-tx-status: { state: 'auto-rx' }`; tapping it
  // sends `enabled: false`. (Gap 14.)
  'sstv-set-auto-enabled': { dir: Dir.C2S, feature: 'sstv', fields: { enabled: f.boolean } },

  // Mobile-triggered audio reset. Same effect as the desktop's
  // Settings → ECHOCAT → "Restart audio" button: rebuild the WebRTC
  // bridge + nudge JTCAT to re-grab its DAX capture. Recovers from
  // Windows RDP audio-shuffle without touching the shack PC.
  'restart-audio': { dir: Dir.C2S, feature: 'core' },
  'restart-audio-result': { dir: Dir.S2C, feature: 'core', fields: { ok: f.boolean, error: f.string, note: f.string } },
  // Audio-bridge health push. Desktop emits ok:false with a reason
  // when the rig→phone audio source goes silent for >5s; emits ok:true
  // when audio recovers. iOS subscribes to drive auto-restart with
  // cool-down + circuit-breaker safety net.
  'audio-health': { dir: Dir.S2C, feature: 'core', fields: { ok: f.boolean, reason: f.string, since: f.number } },

  // ─── Diagnostics (Unified Bug Report) ───────────────────────────────
  // Canonical contract: status/brief-bug-report-{desktop,mobile}.md. A
  // "Report a Bug" tap on EITHER side gathers its own state, requests a
  // snapshot from the other side, and assembles one shareable report.
  // BIDIRECTIONAL — either side can be requester or responder — so both
  // types are Dir.BOTH (same as hello/signal/vfo-profiles). Do NOT pair
  // them as two one-direction types.
  //
  // `redact:true` => the snapshot is safe to paste into a PUBLIC report
  // (email masked, IPs to /24, JWT/Bearer/long-token strings stripped).
  //
  // `sections` is an any-bag object: its shape (account/connection/
  // pairedDevices/rig/tailscale/cloudTunnel/logLines, plus the mobile-only
  // `network`) mirrors the mobile BugReportAssembler HostSnapshot and
  // evolves independently, so it is intentionally untyped here. Every
  // field except requestId is optional so a refused/failed gather can
  // reply with `error` and no sections rather than going silent (which
  // would strand the requester on its 5s timeout). `platform` is an
  // object {os, osVersion, deviceModel}; `timestamp` is an ISO-8601 string.
  'request-diagnostic': { dir: Dir.BOTH, feature: 'core', fields: { requestId: f.string, redact: f.optBoolean } },
  'diagnostic-snapshot': { dir: Dir.BOTH, feature: 'core', fields: { requestId: f.string, source: f.optString, appVersion: f.optString, platform: f.optObject, timestamp: f.optString, sections: f.optObject, error: f.optString } },

  // ─── Cloud (Cognito QSO sync) ───────────────────────────────────────
  'cloud-login': { dir: Dir.C2S, feature: 'cloud' },
  'cloud-login-result': { dir: Dir.S2C, feature: 'cloud' },
  'cloud-register': { dir: Dir.C2S, feature: 'cloud' },
  'cloud-register-result': { dir: Dir.S2C, feature: 'cloud' },
  'cloud-logout': { dir: Dir.C2S, feature: 'cloud' },
  'cloud-logout-result': { dir: Dir.S2C, feature: 'cloud' },
  'cloud-get-status': { dir: Dir.C2S, feature: 'cloud' },
  'cloud-status': { dir: Dir.S2C, feature: 'cloud' },
  'cloud-sync-now': { dir: Dir.C2S, feature: 'cloud' },
  'cloud-sync-result': { dir: Dir.S2C, feature: 'cloud' },
  'cloud-bulk-upload': { dir: Dir.C2S, feature: 'cloud' },
  'cloud-upload-result': { dir: Dir.S2C, feature: 'cloud' },
  'cloud-verify-subscription': { dir: Dir.C2S, feature: 'cloud' },
  'cloud-verify-result': { dir: Dir.S2C, feature: 'cloud' },
  'cloud-save-bmac-email': { dir: Dir.C2S, feature: 'cloud', fields: { bmacEmail: f.string } },
  'cloud-bmac-result': { dir: Dir.S2C, feature: 'cloud' },

  // ─── KiwiSDR / WebSDR ───────────────────────────────────────────────
  // `password` is forwarded to KiwiSDR servers that require auth (most are
  // open). See main.js:6230. (Gap 20b.)
  'kiwi-connect': { dir: Dir.C2S, feature: 'kiwi', fields: { host: f.string, password: f.optString } },
  'kiwi-disconnect': { dir: Dir.C2S, feature: 'kiwi' },
  // QSY the SDR receiver while connected. Mobile freq input wired to this.
  'kiwi-tune': { dir: Dir.C2S, feature: 'kiwi', fields: { freqKhz: f.string, mode: f.optString } },
  // Connection state + S-meter pushed back so mobile can render the SDR card.
  'kiwi-status': { dir: Dir.S2C, feature: 'kiwi', fields: { connected: f.boolean, host: f.optString, error: f.optString } },
  // Other-VFO frequency while split is on (TX side; 0 = split off/hide).
  'freq-other': { dir: Dir.S2C, feature: 'rig', fields: { value: f.optNumber } },
  'kiwi-smeter': { dir: Dir.S2C, feature: 'kiwi', fields: { dbm: f.number } },
  // Raw PCM frames. WebSDR/Kiwi sample rates vary (8000..14238 Hz). Mobile
  // routes through WebRTC instead of decoding directly — this entry exists
  // for the desktop-renderer + ECHOCAT browser path.
  'kiwi-audio': { dir: Dir.S2C, feature: 'kiwi', fields: { pcm: f.array, sampleRate: f.number } },

  // ─── Propagation (RBN + PSKReporter Map) ────────────────────────────
  // Server pushes the full RBN spot array (where the user is being heard)
  // and PSKReporter Map spots (FT8/digital reception reports of the user).
  // Mirrors what the local Propagation popout receives. (Gaps 15, 19.)
  'rbn-prop-spots':  { dir: Dir.S2C, feature: 'prop', fields: { spots: f.array } },
  'pskr-map-spots':  { dir: Dir.S2C, feature: 'prop', fields: { spots: f.array } },
  'pskr-map-status': { dir: Dir.S2C, feature: 'prop', fields: { connected: f.boolean, spotCount: f.optNumber, nextPollAt: f.optNumber, pollUpdate: f.optBoolean } },

  // ─── Activity ("now") feed + idle-session results ───────────────────
  // What the station is doing right now — the router for a mobile app
  // that opens INTO the running activity (idle WSPR, JS8, an SSTV decode
  // worth waiting out). Cached and hydrated FIRST in the connect burst so
  // the client can route before content arrives. One PRIMARY activity by
  // design; a `secondary` field can grow later without breaking the shape.
  // activity: 'idle'|'jtcat'|'js8'|'psk31'|'wspr'|'sstv'|'freedv'.
  // busy answers "should I wait": {tx, decoding, progress?, etaMs?}.
  'activity-state': { dir: Dir.S2C, feature: 'activity', fields: {
    activity: f.string, auto: f.boolean, since: f.number,
    detail: f.optObject, busy: f.optObject,
  } },
  // The WSPR session's ACCUMULATED spots (cap 500) — what the desktop's
  // popout shows after an hour of idle WSPR, not just the last 2-minute
  // batch. jtcat-wspr-spots keeps its shipped replace-the-list semantics;
  // this is the new session-scope feed. active:false = the session ended
  // (mode changed away) but its results remain serveable.
  // today = the per-UTC-day rollup ({date, decoded, uniqueCalls, bestDxMi,
  // bestDxCall, uploadedWsprnet, sentPskr}). Field-name honesty is load-
  // bearing: wsprnet uploads are request/response (ACCEPTED); PSKReporter
  // is fire-and-forget UDP (SENT — there is no ack; never render
  // "received").
  'wspr-session': { dir: Dir.S2C, feature: 'activity', fields: {
    startedAt: f.number, count: f.integer, active: f.boolean, spots: f.array,
    today: f.optObject,
  } },

  // ─── JS8 (native HF messaging) ──────────────────────────────────────
  // JS8 is decoded/transmitted by POTACAT itself (lib/js8-engine.js); the
  // phone is a full peer of the desktop JS8 window: same conversation
  // store, same unread counts, same groups. Conversation state lives in
  // MAIN — the phone renders it, never owns it.
  //
  // Guest Pass: js8-start/js8-stop/js8-heartbeat/js8-send are REFUSED for
  // guests (they key or configure the host's transmitter); browsing threads
  // is allowed (RX-only). A refusal answers with js8-send-result
  // {ok:false, error} so the guest's UI shows why instead of doing nothing.
  // reqId (declared, not merely tolerated — mobile ask 2026-08-09) lets a
  // host-side start failure answer on js8-send-result attributed to the tap.
  'js8-start': { dir: Dir.C2S, feature: 'js8', fields: { reqId: f.optString } },
  'js8-stop': { dir: Dir.C2S, feature: 'js8', fields: { reqId: f.optString } },
  // enabled toggles the scheduler (session-only on the host, 30-min attended
  // watchdog); intervalMin (5-60) persists. Either field alone is valid.
  // enabled:true also TRANSMITS one heartbeat immediately (a button named HB
  // that only arms a timer is not a heartbeat) — expect js8-state.txQueue to
  // tick right after enabling.
  'js8-heartbeat': { dir: Dir.C2S, feature: 'js8', fields: { enabled: f.optBoolean, intervalMin: f.optInteger, reqId: f.optString } },
  // Send ONE heartbeat now (momentary, repeatable) — distinct from the
  // js8-heartbeat scheduler toggle. Guest-refused (keys the rig).
  'js8-send-hb': { dir: Dir.C2S, feature: 'js8', fields: { reqId: f.optString } },
  // ⚙ opt-ins. HB ACK arms automatic TX (guest-refused, session-only on the
  // host); the APRS gate is a persisted internet-only setting (guest-refused —
  // it publishes under the OWNER's callsign).
  'js8-set-hback': { dir: Dir.C2S, feature: 'js8', fields: { enabled: f.optBoolean } },
  'js8-set-aprs-gate': { dir: Dir.C2S, feature: 'js8', fields: { enabled: f.optBoolean } },
  // Which program the station launches when idle (Auto-RX): sstv | wspr |
  // psk31 | js8. Persisted; echoed via the settings blob (idleRxMode).
  'set-idle-rx': { dir: Dir.C2S, feature: 'js8', fields: { mode: f.optString } },
  // Outbound SMS/email via APRS (owner-only): host builds the padded packet
  // + sequence from settings-configured gateways; result rides js8-send-result.
  'js8-send-sms': { dir: Dir.C2S, feature: 'js8', fields: { kind: f.optString, to: f.optString, text: f.optString, reqId: f.optString } },
  // Reachability, the other direction (heartbeat map): stations whose SNR
  // replies prove they hear US. Same shape as js8-heard.
  'js8-heard-by': { dir: Dir.S2C, feature: 'js8', fields: { list: f.optArray } },
  // Mail (DM-class, owner-only): one message on store, full list at connect,
  // read-marks from the phone.
  'js8-mail': { dir: Dir.S2C, feature: 'js8', fields: { id: f.optString, from: f.optString, text: f.optString, utc: f.optNumber } },
  'js8-mail-list': { dir: Dir.S2C, feature: 'js8', fields: { messages: f.optArray } },
  'js8-mail-read': { dir: Dir.C2S, feature: 'js8', fields: { id: f.optString } },
  // Joined @NETS from the phone (portable operating; mobile RESPONSE §5).
  'js8-set-groups': { dir: Dir.C2S, feature: 'js8', fields: { groups: f.optString } },
  // Transmit. `to` is the destination (callsign or @GROUP) — the HOST
  // composes the addressed form, so the phone never learns the "@GROUP text"
  // vs "CALL: text" rules. reqId (client-chosen) rides back on the result so
  // the phone can match refusals to taps.
  'js8-send': { dir: Dir.C2S, feature: 'js8', fields: { text: f.string, to: f.optString, reqId: f.optString } },
  'js8-send-result': { dir: Dir.S2C, feature: 'js8', fields: { ok: f.boolean, error: f.optString, text: f.optString, frames: f.optInteger, reqId: f.optString } },
  // Opening a thread marks it read on the HOST (the one unread truth) and
  // answers with js8-thread. Closing releases the auto-read claim.
  'js8-thread-open': { dir: Dir.C2S, feature: 'js8', fields: { id: f.string } },
  'js8-thread-closed': { dir: Dir.C2S, feature: 'js8' },
  // Log a conversation: C2S {id} asks the HOST to extract the QSO from the
  // thread (lib/js8-qso.js — reports each way, latest-exchange times, grid,
  // dial+offset frequency); S2C replies with {id, prefill} (prefill null +
  // error when there is nothing to log). The phone opens its own log form
  // with the prefill and submits via the existing log-qso channel, so
  // uuid/cloud-merge/session-contacts apply unchanged. Refused for guests
  // (it writes the owner's logbook).
  'js8-log-prefill': { dir: Dir.BOTH, feature: 'js8', fields: { id: f.string, prefill: f.optObject, error: f.optString, reqId: f.optString } },
  'js8-thread': { dir: Dir.S2C, feature: 'js8', fields: { thread: f.optObject } },
  // Tune the rig to a band's JS8 dial (14078 kHz and friends — the host
  // owns the table). Refused for guests. The result shows up as
  // js8-state.band/dialHz plus the normal radio-status stream.
  'js8-set-band': { dir: Dir.C2S, feature: 'js8', fields: { band: f.string, reqId: f.optString } },
  // Engine + station snapshot. Cached server-side and hydrated at connect.
  // station = { call, grid }. txQueue = frames waiting for their period.
  // dialHz/band (2026-08-09): where the rig is — band-granular pushes on
  // QSY (the Hz-live stream stays on the radio-status broadcast).
  'js8-state': { dir: Dir.S2C, feature: 'js8', fields: { running: f.boolean, tx: f.optBoolean, txQueue: f.optInteger, submode: f.optString, heartbeat: f.optBoolean, heartbeatMin: f.optInteger, station: f.optObject, dialHz: f.optNumber, band: f.optString, txTotal: f.optInteger, hbNextAt: f.optNumber, hbAck: f.optBoolean, swrAutoTune: f.optBoolean, aprsGate: f.optBoolean, aprsGateUp: f.optBoolean, txOffset: f.optNumber, rxOffset: f.optNumber, swrTripped: f.optBoolean, swrMessage: f.optString, groups: f.optArray, mailUnread: f.optInteger } },
  // Conversation list push: list rows are the store's list() shape
  // ({id, call, isGroup, unread, lastUtc, lastText, lastDir, hbCount,
  // count}); unread is the total badge; when one thread changed, changed
  // carries its id and thread the full content so an open view updates
  // without a round trip. Cached + hydrated at connect.
  'js8-threads': { dir: Dir.S2C, feature: 'js8', fields: { list: f.array, unread: f.integer, changed: f.optString, thread: f.optObject } },
  // Stations audible now ({call, snr, utc, grid}), newest first, cap 40.
  'js8-heard': { dir: Dir.S2C, feature: 'js8', fields: { list: f.array } },

  // ─── Voice macros / preferences / decorations ───────────────────────
  // voice-macro-sync flows BOTH ways: phone uploads a recording (audio is
  // base64-encoded WebM), and at startup the server pushes existing
  // recordings down to the phone the same shape. Earlier the schema only
  // declared C2S. (Gap 3.)
  'voice-macro-sync': { dir: Dir.BOTH, feature: 'misc', fields: { idx: f.integer, label: f.optString, audio: f.optString } },
  'voice-macro-delete': { dir: Dir.C2S, feature: 'misc', fields: { idx: f.integer } },
  // Mobile macro-button tap → desktop plays the recorded clip out the rig
  // via the existing local voice-macro-ptt path. No payload back; TX state
  // surfaces via the regular status broadcast. (Gap 18.)
  'voice-macro-play': { dir: Dir.C2S, feature: 'misc', fields: { idx: f.integer } },

  // JTCAT: hold TX freq toggle. When on, AUTO setTxFreq() calls (QSO state
  // machine, reply station-follow, quiet-freq pickers) are ignored — the
  // pinned TX freq stays put while RX still tracks responders. Deliberate
  // operator moves (jtcat-set-tx-freq / popout TX box / waterfall) are
  // honored and become the new pinned value (WSJT-X "Hold Tx Freq" parity,
  // KF0U 2026-07-17). K0OTC 2026-05-04: requested for fixed-freq park
  // operation while still using auto-Seq for QSO completion.
  'jtcat-set-hold-tx-freq': { dir: Dir.C2S, feature: 'jtcat', fields: { enabled: f.boolean } },
  'jtcat-hold-tx-state':    { dir: Dir.S2C, feature: 'jtcat', fields: { enabled: f.boolean } },
  // voice-macro-labels: server pushes the 5-slot label array on connect,
  // and again whenever the desktop UI saves new labels. (Gap 2.)
  'voice-macro-labels': { dir: Dir.S2C, feature: 'misc', fields: { labels: f.array } },
  'save-echo-pref': { dir: Dir.C2S, feature: 'misc' },
  'save-custom-cat-buttons': { dir: Dir.C2S, feature: 'misc', fields: { buttons: f.array } },
  'colorblind-mode': { dir: Dir.S2C, feature: 'misc', fields: { enabled: f.boolean } },
  'cluster-state': { dir: Dir.S2C, feature: 'misc', fields: { connected: f.boolean } },
  // qrz-names: server pushes a {CALLSIGN: 'Display Name'} map after each
  // batch QRZ lookup so the phone can label spot rows without doing its
  // own QRZ calls. (Gap 7.)
  'qrz-names': { dir: Dir.S2C, feature: 'misc', fields: { data: f.object } },
});

/**
 * First-message types accepted from a *legacy* (pre-v1) client. When a
 * client's first frame is one of these instead of `hello`, the server
 * should fall back to v0 behavior — i.e. skip the version handshake and
 * keep the connection alive, but mark capabilities as v0.
 *
 * The legacy browser ECHOCAT sends `auth` first. Keep that working
 * forever; native clients are expected to lead with `hello`.
 */
const LEGACY_FIRST_MESSAGE_TYPES = Object.freeze(['auth']);

/**
 * Return true if `type` is registered.
 */
function isKnownType(type) {
  return Object.prototype.hasOwnProperty.call(MESSAGES, type);
}

/**
 * Return the registry entry for `type` (or undefined).
 */
function describe(type) {
  return MESSAGES[type];
}

/**
 * Validate a parsed message object.
 *
 * Returns `{ ok: true }` on success, or
 * `{ ok: false, error: string, field?: string }` on failure.
 *
 * `expectedDir` is optional. When provided ('s2c' or 'c2s'), the
 * validator will refuse messages whose registered direction is the
 * opposite (i.e. a server that receives a `spots` push from a client
 * is misbehaving and we want to catch it).
 */
function validate(msg, expectedDir) {
  if (msg == null || typeof msg !== 'object') {
    return { ok: false, error: 'message must be an object' };
  }
  if (typeof msg.type !== 'string' || msg.type.length === 0) {
    return { ok: false, error: 'message.type missing or not a string' };
  }
  const def = MESSAGES[msg.type];
  if (!def) return { ok: false, error: 'unknown message type', field: 'type' };

  if (expectedDir && def.dir !== Dir.BOTH && def.dir !== expectedDir) {
    return { ok: false, error: `message ${msg.type} not allowed in direction ${expectedDir}` };
  }

  if (def.fields) {
    for (const [name, spec] of Object.entries(def.fields)) {
      const required = spec.required !== false;
      // Treat explicit `null` the same as absent (standard JSON-over-
      // wire semantics). Senders emit unset optional fields as `null`
      // (e.g. jtcat-wspr-spots carries `error: null` on a successful
      // batch); counting null as "present" then type-checking it as a
      // string silently DROPPED the whole message in parse(). A null
      // required field still fails below (as "required"). Mirrored in
      // the mobile port src/protocol/echocatProtocol.ts.
      const present = Object.prototype.hasOwnProperty.call(msg, name) && msg[name] !== undefined && msg[name] !== null;
      if (!present) {
        if (required) return { ok: false, error: `field ${name} is required`, field: name };
        continue;
      }
      const value = msg[name];
      const tcheck = checkType(value, spec);
      if (!tcheck.ok) return { ok: false, error: `field ${name}: ${tcheck.error}`, field: name };
    }
  }
  return { ok: true };
}

function checkType(value, spec) {
  switch (spec.type) {
    case 'any':
      return { ok: true };
    case 'string':
      return typeof value === 'string'
        ? { ok: true }
        : { ok: false, error: 'expected string' };
    case 'number':
      return typeof value === 'number' && Number.isFinite(value)
        ? { ok: true }
        : { ok: false, error: 'expected finite number' };
    case 'integer':
      return Number.isInteger(value)
        ? { ok: true }
        : { ok: false, error: 'expected integer' };
    case 'boolean':
      return typeof value === 'boolean'
        ? { ok: true }
        : { ok: false, error: 'expected boolean' };
    case 'array':
      return Array.isArray(value)
        ? { ok: true }
        : { ok: false, error: 'expected array' };
    case 'object':
      return value && typeof value === 'object' && !Array.isArray(value)
        ? { ok: true }
        : { ok: false, error: 'expected object' };
    case 'oneOf':
      return spec.values.includes(value)
        ? { ok: true }
        : { ok: false, error: `expected one of [${spec.values.join(',')}]` };
    default:
      return { ok: false, error: `unknown spec type ${spec.type}` };
  }
}

/**
 * Parse + validate a raw text frame from the wire.
 *
 * Returns `{ ok: true, msg }` or `{ ok: false, error }`.
 */
function parse(rawText, expectedDir) {
  let msg;
  try {
    msg = JSON.parse(rawText);
  } catch {
    return { ok: false, error: 'invalid JSON' };
  }
  const v = validate(msg, expectedDir);
  if (!v.ok) return { ok: false, error: v.error, field: v.field, raw: msg };
  return { ok: true, msg };
}

/**
 * Encode a message object to the wire format. Validates first; throws
 * on a malformed message rather than silently sending garbage.
 *
 * Server sites that broadcast structured-but-untyped payloads can pass
 * `{skipValidate: true}` to opt out — we do this for `status` and a
 * handful of pass-through messages whose shapes haven't been pinned
 * yet. Audit and remove these escapes over time.
 */
function encode(msg, opts = {}) {
  if (!opts.skipValidate) {
    const v = validate(msg);
    if (!v.ok) {
      const err = new Error(`echocat-protocol: refused to encode invalid message: ${v.error}`);
      err.code = 'PROTOCOL_INVALID';
      err.field = v.field;
      throw err;
    }
  }
  return JSON.stringify(msg);
}

/**
 * Server hello builder. Server sends this immediately after the WS
 * upgrade — before auth-mode — for any client that has indicated
 * (via a `hello` first frame) that it speaks v1+.
 */
function buildServerHello(opts) {
  return {
    type: 'hello',
    protocolVersion: PROTOCOL_VERSION,
    serverVersion: opts && opts.serverVersion ? String(opts.serverVersion) : '',
    capabilities: opts && opts.capabilities ? opts.capabilities : [],
    rigModel: opts && opts.rigModel ? String(opts.rigModel) : '',
  };
}

/**
 * Client hello builder. Native and CLI clients send this first.
 * Browser ECHOCAT does not, and falls back to legacy v0 mode.
 */
function buildClientHello(opts) {
  return {
    type: 'hello',
    protocolVersion: PROTOCOL_VERSION,
    clientVersion: opts && opts.clientVersion ? String(opts.clientVersion) : '',
    clientPlatform: opts && opts.clientPlatform ? String(opts.clientPlatform) : '',
    // Capability advertisement (Architecture B, v1.9). Lets the host
    // decide whether to forward auto-logged QSOs (qso-attributed) or
    // surface log-error per the hard rule. The hello schema already
    // declares capabilities as f.optArray, so this is additive.
    capabilities: Array.isArray(opts && opts.capabilities) ? opts.capabilities : [],
  };
}

/**
 * Decide compatibility between a remote `hello` and the local
 * PROTOCOL_VERSION. Both sides should call this with the peer's
 * advertised version.
 *
 * Returns:
 *   - { compatible: true }                        — same major
 *   - { compatible: true, downgrade: true }       — peer is older but within range
 *   - { compatible: false, reason: string }       — close the socket
 */
function checkCompatibility(peerVersion) {
  if (typeof peerVersion !== 'number' || !Number.isInteger(peerVersion) || peerVersion < 0) {
    return { compatible: false, reason: 'peer protocolVersion missing or not an integer' };
  }
  if (peerVersion === PROTOCOL_VERSION) return { compatible: true };
  if (Math.abs(peerVersion - PROTOCOL_VERSION) > 1) {
    return {
      compatible: false,
      reason: `peer protocolVersion ${peerVersion} is too far from ours (${PROTOCOL_VERSION})`,
    };
  }
  return { compatible: true, downgrade: peerVersion < PROTOCOL_VERSION };
}

module.exports = {
  PROTOCOL_VERSION,
  CLOSE_CODES,
  Dir,
  MESSAGES,
  LEGACY_FIRST_MESSAGE_TYPES,
  isKnownType,
  describe,
  validate,
  parse,
  encode,
  buildServerHello,
  buildClientHello,
  checkCompatibility,
};
