...
function encodeStatus(id, status) {
  const w = new BinaryWriter();
  buildHeader(w, MSG.STATUS, id);
  w.writeUInt64BE(status.dialFrequency || 0);
  w.writeUtf8(status.mode || '');
  w.writeUtf8(status.dxCall || '');
  w.writeUtf8(status.report || '');
  w.writeUtf8(status.txMode || status.mode || '');
  w.writeBool(!!status.txEnabled);
  w.writeBool(!!status.transmitting);
  w.writeBool(!!status.decoding);
  w.writeInt32BE(status.rxDF || 0);
  w.writeInt32BE(status.txDF || 0);
  w.writeUtf8(status.deCall || '');
  w.writeUtf8(status.deGrid || '');
  w.writeUtf8(status.dxGrid || '');
  w.writeBool(!!status.txWatchdog);
  w.writeUtf8(status.subMode || '');
  w.writeBool(!!status.fastMode);
  w.writeUInt8(status.specialOpMode || 0);
  w.writeUInt32BE(status.frequencyTolerance || 20);
  w.writeUInt32BE(status.trPeriod || 15);
  w.writeUtf8(status.configName || 'POTACAT JTCAT');
  return w.toBuffer();
}

function encodeDecode(id, decode) {
  const w = new BinaryWriter();
  buildHeader(w, MSG.DECODE, id);
  w.writeBool(decode.isNew !== false);
  w.writeQTime(decode.time != null ? decode.time : Date.now() % 86400000);
  w.writeInt32BE(decode.snr || 0);
  w.writeDoubleBE(decode.deltaTime || 0);
  w.writeUInt32BE(decode.deltaFrequency || 0);
  w.writeUtf8(decode.mode || '');
  w.writeUtf8(decode.message || '');
  w.writeBool(!!decode.lowConfidence);
  w.writeBool(!!decode.offAir);
  return w.toBuffer();
}
...
function extractCallsigns(message) {
  ...
  if (parts[0] === 'CQ') {
    // "CQ K1ABC FN42" or "CQ DX K1ABC FN42" or "CQ NA K1ABC FN42"
    // CQ modifiers are all-alpha, 2-4 chars, no digits (DX, NA, EU, POTA, etc.)
    const isCqModifier = parts[1] && /^[A-Z]{2,4}$/.test(parts[1]) && !/\d/.test(parts[1]);
    const callIdx = isCqModifier ? 2 : 1;
...
module.exports = { WsjtxClient, extractCallsigns, MSG, BinaryWriter, buildHeader, encodeHeartbeat, encodeStatus, encodeDecode, encodeLoggedAdif, encodeQsoLogged };
...
function encodeQsoLogged(id, qso) {
  ...
  w.writeUtf8(qso.propMode || '');                 // ADIF propagation mode (schema >= 2)
  return w.toBuffer();
}
