...
/**
 * True when a decode is addressed TO us: first token is OUR call, and the
 * second is the partner's (when known). The old contains-both-calls matchers
 * were order-blind, so a decode of our OWN transmission ("AD9AR N3VD -11" —
 * an IC-7300 with MONI on loops TX audio back over USB and the decoder
 * happily decodes it) matched as the partner's response; the optional-R
 * report regex then read our own report back and auto-logged sent==rcvd with
 * no QSO actually completed (N3VD, 8-in-a-row, 2026-07-10). Their messages
 * to us are always "MYCALL THEIRCALL …", so anchoring the order is exact.
 */
function _addressedToUs(text, myCall, theirCall) {
  const tokens = _normText(text).trim().split(/\s+/);
  if (tokens[0] !== _normText(myCall)) return false;
  return !theirCall || tokens[1] === _normText(theirCall);
}
...
function advanceJtcatQso(q, results, setTxMsg, onDone, deps) {
  ...
  if (q.mode === 'cq') {
    ...
    if (q.phase === 'cq') {
      // deps.skipCall lets the caller veto an answerer it has already worked.
      // Run mode needs this: without it a worked station answering a later CQ
      // gets worked again, so the band never actually "drains" and the pause
      // below could never trigger. A reply to us is `MYCALL THEIRCALL …`, so
      // their call is the second token. No skipCall (manual/hunt paths) keeps
      // the original take-the-first-answerer behavior.
      const skipCall = typeof deps.skipCall === 'function' ? deps.skipCall : null;
      let skipped = 0;
      const reply = results.find((d) => {
        const t = _normText(d.text);
        if (t.indexOf(myCall) < 0 || t.startsWith('CQ ')) return false;
        if (skipCall) {
          const their = t.split(/\s+/)[1];
          if (their && skipCall(their)) { skipped++; return false; }
        }
        return true;
      });
      if (!reply) {
        _qsoLog('no advance', skipped
          ? `(${skipped} answerer(s) already worked, skipped)`
          : `(no decode containing ${myCall} that isn't a CQ)`);
        return;
      }
      ...
      // RR73 satisfies [A-R]{2}\d{2} but is not a grid (WSJT-X reserves it as
      // the impossible locator for exactly this reason) — a straggler RR73
      // from a finished QSO must not start a phantom new one.
      if (m && m[2].toUpperCase() !== 'RR73') {
        q.call = m[1];
        q.grid = m[2];
    ...
    if (q.phase === 'cq-report') {
      // Addressed-to-us only — an order-blind contains-both-calls match let
      // our own echoed TX ("THEIRCALL MYCALL -NN") advance and log here.
      const resp = results.find((d) => _addressedToUs(d.text, myCall, q.call));
      if (!resp) {
        _qsoLog('no advance', `(no decode addressed ${myCall} ${q.call})`);
        return;
      }
  ...
  const resp = results.find((d) => {
    const t = _normText(d.text);
    // Hound: first-token-only. The fox's RR73 segment omits its own call
    // ("K3SBP RR73; W2ABC <KH7Z> -04") and its sign-off may be the compound
    // form while we track the base call — requiring token[2]==theirCall
    // would drop both. Own echoes still can't pass: our hound TX starts
    // with the FOX's call, never ours.
    if (q.hound) {
      if (t.indexOf(';') >= 0) return t.split(';').some((s) => _addressedToUs(s, myCall, null));
      return _addressedToUs(t, myCall, null);
    }
    // Addressed-to-us only (see _addressedToUs) — with Skip Grid our own
    // echoed opener "THEIRCALL MYCALL -NN" carries a report and would
    // otherwise advance the reply phase with our own report as theirs.
    return _addressedToUs(t, myCall, theirCall);
  });
  if (!resp) {
...
function advanceFdReplyQso(q, results, setTxMsg, onDone, engine, _qsoLog) {
  ...
  const resp = results.find((d) => _addressedToUs(d.text, myCall, theirCall));
  if (!resp) {
...
/**
 * Should run mode stop calling CQ and just listen?
 *
 * Deliberately a sibling of decideRetryOutcome rather than a change to it:
 * that function's "cq + runMode → continue forever" rule is what keeps a CQ
 * from ever aborting, and it stays exactly as it was.
 *
 * BOTH conditions must hold. Unanswered CQs alone would pause on a busy band
 * where the operator is simply not being heard — stations are around, so
 * keeping the CQ going is right. No workable callers alone would pause during
 * a brief lull mid-pileup. It is the combination that means "drained".
 *
 * @param {object} o
 * @param {number} o.unansweredCqs   consecutive CQ transmissions with no answer
 * @param {number} o.maxUnanswered   settings.jtcatRunPauseAfter (0 = never pause)
 * @param {number} o.workableCallers unworked stations calling CQ in this cycle
 * @returns {{action:'continue'|'pause'}}
 */
function decideRunCqPause(o) {
  const max = o.maxUnanswered || 0;
  if (max <= 0) return { action: 'continue' };
  if ((o.unansweredCqs || 0) < max) return { action: 'continue' };
  if ((o.workableCallers || 0) > 0) return { action: 'continue' };
  return { action: 'pause' };
}

/**
 * Gate for the per-phase try counter: a "try" is a TRANSMISSION, not a
 * decode period. Decode events land every period — including the one we
 * transmitted in (whose reply window hasn't been heard yet) and periods
 * where TX was deferred/held — and counting each of those burned the cap
 * twice per QSO cycle: "tries 5" gave up after the 3rd transmission
 * (KQ4MHD 2026-08-04). Count exactly once per TX, on a LATER period so
 * that TX's reply window has been listened to first.
 *
 * @param {object} o
 * @param {string} o.periodKey            the period being evaluated (jtcatPeriodUtc)
 * @param {string} o.txPeriodKey          period of the engine's last real TX ('' = none yet)
 * @param {string} o.lastCountedTxPeriod  txPeriodKey already counted for this QSO
 * @returns {boolean} true when this evaluation should count a try
 */
function shouldCountRetry(o) {
  if (!o.txPeriodKey) return false;                        // never transmitted yet
  if (o.txPeriodKey === o.periodKey) return false;         // TX period's own decode — reply still pending
  if (o.txPeriodKey === o.lastCountedTxPeriod) return false; // this TX already counted
  return true;
}
