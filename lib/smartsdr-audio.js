...
class SmartSdrAudio extends EventEmitter {
  ...
  _fireSetupSequence(udpPort) {
    ...
    // 6. Subscribe to transmit status ON THIS CONNECTION. Transmit config is
    //    per-station (probe 2026-08-03: bound read dax=1 while an unbound
    //    read of the same radio said dax=0), so the dax= readback must come
    //    from the same bound context setTxDax() writes to — the main API
    //    connection is unbound in bound mode and can read the wrong station.
    this._txDaxOn = null;
    const seqSubTx = send('sub tx all');
    if (seqSubTx != null) this._cmdSeqs.add(seqSubTx);
  }
  ...
  /**
   * Select the radio's TX audio source (global `transmit set dax=`): true =
   * the DAX TX stream (this connection's direct VITA-49 feed), false = mic.
   * Sent on THIS connection deliberately: Flex transmit config is per-station,
   * and an unbound client's `transmit set dax` is silently ignored (probe
   * evidence 2026-08-03 — the radio re-broadcast dax=0 after the set). This
   * connection is always `client bind`-ed before streams exist (self mode:
   * POTACAT's own GUI client; bound mode: the discovered SmartSDR/AetherSDR
   * client), so the set lands in the station context that owns our dax_tx
   * stream. The main API connection only binds when the CW keyer needs it.
   */
  setTxDax(on) {
    const seq = this._send(`transmit set dax=${on ? 1 : 0}`);
    if (seq != null) this.emit('log', `Audio TX: C${seq}|transmit set dax=${on ? 1 : 0}`);
    return seq != null;
  }
  ...
  _handleLine(line) {
    // Transmit status (from our `sub tx all`) — we only care about dax=,
    // the per-station TX-audio-source switch this connection asserts around
    // POTACAT transmissions. Delta lines without dax= leave state unchanged.
    const txMatch = line.match(/\|transmit (.+)$/);
    if (txMatch) {
      const daxKv = txMatch[1].match(/(?:^| )dax=([01])/);
      if (daxKv) this._txDaxOn = daxKv[1] === '1';
      return;
    }
  ...
  /**
   * Hand the station's DAX transmit path to another application.
   *
   * `transmit set dax=1` selects DAX as the TX audio source for the STATION,
   * and the station uses the dax_tx stream it owns — ours. While POTACAT holds
   * one, an application feeding the Windows DAX TX device (JS8Call through the
   * DAX control panel) is displaced, and keying produces a dead carrier: full
   * power, no modulation, no error (K3SBP 2026-08-07, JS8Call heartbeat).
   *
   * Removing our stream leaves the DAX app's as the only one, so its audio is
   * what the transmitter takes. Returns true if a stream was actually given up,
   * so the caller knows whether to put it back.
   */
  releaseTxStream() {
    if (!this._txStreamId || !this._connected) return false;
    const id = this._txStreamId;
    try { this._send(`stream remove 0x${id.toString(16)}`); } catch { return false; }
    this._streamIds.delete(id);
    this._txStreamId = null;
    this.emit('log', `DAX TX stream 0x${id.toString(16)} released — another application owns transmit audio`);
    return true;
  }

  /** Take the DAX transmit path back. POTACAT's own TX needs it. */
  restoreTxStream() {
    if (this._txStreamId || !this._connected) return false;
    const seq = this._send('stream create type=dax_tx');
    if (seq == null) return false;
    this._txCreateSeq = seq;
    this._cmdSeqs.add(seq);
    this.emit('log', 'DAX TX stream requested back');
    return true;
  }

  /** Last-known transmit dax= for THIS connection's bound station context —
   *  true/false, or null before the first transmit status arrives. */
  ...
  pushTxAudioChunk(mono24k) {
    ...
    // Never interleave the live mic stream with an in-flight one-shot envelope
    // (FT8/FT4 TX or Tune via sendTxAudio). Both writers share the dax_tx
    // stream id AND the 4-bit packet counter, so concurrent sends delivered
    // 2× real-time samples and the radio played the FT8 envelope diluted
    // ~50% with silence — garbled on air, plus a ~13 s TX-buffer backlog
    // that drained past PTT-off and wrecked the following RX slot.
    // (K3SBP 2026-06-11, guest-pass iPad session: audio bridge live during
    // engine TX was the first time both paths ran concurrently.)
    if (this._txInFlight) return;
  ...
  /** Drop any partially-accumulated streaming TX audio (e.g. when JTCAT Tune
   *  stops). The drain loop only emits full 128-frame packets, so a leftover
   *  partial would otherwise prepend a few ms of stale tone to the next stream. */
  resetTxStream() {
    this._streamLen = 0;
  }
