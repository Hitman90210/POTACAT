...
const { PskEngine } = require('./psk-engine');
const { Js8Engine } = require('./js8-engine');
...
class JtcatManager extends EventEmitter {
  ...
  startSlice(config) {
    ...
    // PSK31 and JS8 are different engine classes — Ft8Engine.setMode()
    // would silently coerce their mode strings to 'FT8', so the branch must
    // happen here, not in setMode.
    let engine;
    if (config.mode === 'PSK31') {
      engine = new PskEngine();
    } else if (config.mode === 'JS8') {
      engine = new Js8Engine();
      if (config.submode) engine.setSubmode(config.submode);
      if (config.myCall || config.myGrid) {
        engine.setStation({ call: config.myCall, grid: config.myGrid });
      }
    } else {
      engine = new Ft8Engine();
      engine.setMode(config.mode || 'FT8');
    }

    // Forward all events tagged with sliceId
    for (const evt of ['decode', 'cycle', 'tx-start', 'tx-end', 'status', 'silent', 'psk-text',
                       'js8-rx', 'js8-tx-done', 'encode-failed', 'log']) {
      engine.on(evt, (data) => {
