...
function adifModeSubmode(mode, submode) {
  ...
  // LoTW and most loggers — same failure class the PSK31 mapping fixes. All
  // JS8 speeds are the same ADIF submode; speed is not an ADIF distinction.
  if (!submode && m === 'JS8') return { mode: 'MFSK', submode: 'JS8' };
  return { mode, submode };
}
...
function buildAdifRecord(qso) {
  ...
  const fields = [
    ...
    adifField('MODE', ms.mode),
    adifField('SUBMODE', ms.submode),
    ...
    adifField('CONTEST_ID', qso.contestId),
    adifField('CLASS', qso.class),
    adifField('ARRL_SECT', qso.arrlSect),
    adifField('STX_STRING', qso.stxString),
    adifField('SRX_STRING', qso.srxString),
    ...
    adifField('APP_POTACAT_EVENT', qso.eventId),
    adifField('APP_POTACAT_EVENT_ITEM', qso.eventItem),
...
module.exports = { appendQso, buildAdifRecord, adifField, adifModeSubmode, appendImportedQso, appendRawQso, rewriteAdifFile, writeActivationAdif, writeActivationAdifRaw, ADIF_HEADER };
